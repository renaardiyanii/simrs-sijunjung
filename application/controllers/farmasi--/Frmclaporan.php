...
class Frmclaporan extends Secure_area {
   ...
   //          $total = $sub_total - $row->diskon;
			$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $i);
			$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $row->asal);
			$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $row->tgl_kunjungan);
			$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, $row->no_register);
			$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, $row->no_cm);
			$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, $row->nama);
			$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, $row->cara_bayar);
			$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, $row->nm_dokter);
			$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, $row->ruang_poli);
			$objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, $row->nama_obat);
			$objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, $row->qty);
			$objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, $row->harga);
			$objPHPExcel->getActiveSheet()->SetCellValue('M'.$rowCount, $row->vtot);
