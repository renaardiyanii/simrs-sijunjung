...
and d.userid=$xuser");
		...
		function get_data_kunj_harian($tgl, $id_poli,$cara_bayar,$bayar_bpjs){
			...
			return $this->db->query("SELECT du.no_register, du.no_medrec, dp.nama, dp.no_cm, du.cara_bayar, 
				                    --IFNULL(dp.no_nrp,'-') as no_nrp,
				                     IFNULL(du.ket_pulang,'-') as ket_pulang,
									...
									WHERE LEFT(du.tgl_kunjungan,10) = '$tgl' AND du.id_poli='$id_poli' and du.status = 1 and a.ket_pulang !='BATAL_PELAYANAN_POLI' $textcb $textbb
		...
		function get_data_kunj_bulanan($bulan, $id_poli){
			return $this->db->query("SELECT DATE_FORMAT(LEFT(tgl_kunjungan,10),'%d-%m-%Y') AS tgl_kunj, count(*) AS jumlah_kunj,  COUNT(IF(jns_kunj = 'BARU',1,NULL)) as pasien_baru, COUNT(IF(jns_kunj = 'LAMA',1,NULL)) as pasien_lama, COUNT(IF(ket_pulang = 'BATAL_PELAYANAN_POLI', 1, NULL)) AS jumlah_batal
										...
										and status = 1 and ket_pulang!='BATAL_PELAYANAN_POLI'
		...
		function get_data_kunj_poli_bulanan($bulan){
			return $this->db->query("SELECT id_poli, LEFT ( tgl_kunjungan, 10 ) AS tgl_kunj, count(no_register) AS jumlah_kunj, COUNT(IF(jns_kunj = 'BARU',1,NULL)) as pasien_baru, COUNT(IF(jns_kunj = 'LAMA',1,NULL)) as pasien_lama, COUNT(IF(cara_bayar = 'UMUM',1,NULL)) as umum, COUNT(IF(cara_bayar = 'BPJS',1,NULL)) as bpjs, COUNT(IF(cara_bayar = 'KERJASAMA',1,NULL)) as kerjasama, COUNT(IF(ket_pulang = 'BATAL_PELAYANAN_POLI', 1, NULL)) AS jumlah_batal  
										FROM daftar_ulang_irj 
										WHERE 
										LEFT(tgl_kunjungan,7) = '$bulan'
