...
(SELECT SUM(qty_beli) FROM po WHERE id_po = a.id_po AND qty_beli IS NOT NULL) AS qtybeli
   ...
   //      return $this->db->query("SELECT a.id_po, a.no_po, a.tgl_po, a.supplier_id, s.company_name, a.sumber_dana, a.user, a.open
    ...
    function get_po_detail_list($id){
        return $this->db->query("SELECT p.*, m.hargabeli, m.hargajual, (p.qty * p.`harga_item`) AS subtotal, (p.qty * p.`harga_item`)-((p.qty * p.`harga_item`)*p.diskon_persen/100) AS total
            FROM po p
    ...
    function get_po_detail_beli($data){
        return $this->db->query("SELECT a.id, a.id_po, a.item_id, a.description, a.satuank, a.qty, a.qty_beli, a.expire_date, a.batch_no, a.keterangan, a.hargabeli, a.user, o.hargajual, a.`jml_kemasan`, a.diskon_persen
			FROM po a
    ...
    function get_total_beli($data){
        return $this->db->query("SELECT description, qty, a.satuank, IFNULL(SUM(qty_beli),0) as total_qty_beli, IFNULL(MAX(qty),0) as qty_po, IFNULL(MAX(qty),0)-IFNULL(SUM(qty_beli),0) as kuota, b.open, o.hargajual, a.harga_item AS hargabeli, a.`jml_kemasan`, a.`harga_po`, a.`satuan_item`, a.`diskon_persen`
			FROM po a
