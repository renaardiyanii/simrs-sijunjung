<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
  ...
  function get_all_data_receiving_date($date){
     return $this->db->query("SELECT * , (SELECT SUM(quantity_purchased) FROM receivings_items WHERE
        receiving_id=a.receiving_id GROUP BY receiving_id) as total FROM receivings as a, suppliers as b WHERE a.supplier_id=b.person_id and tgl_kontra_bon='$date'
        ORDER BY a.receiving_id");
  }
  ...
  function get_biaya($id_obat){
	   return $this->db->query("SELECT hargabeli, satuank, satuanb, faktorsatuan FROM master_obat WHERE id_obat='".$id_obat."'");
	}
 ...
 //   print_r($stok);die();
  ...
  function update_mobat($harga_po,$id_obat,$hargabeli, $faktorsatuan, $harga_jual,$margin){
     $this->db->query("UPDATE master_obat SET hargabeli=".$hargabeli.", margin =  ".$margin.", harga_po = ".$harga_po.", faktorsatuan='".$faktorsatuan."',hargajual='".$harga_jual."' WHERE id_obat = ".$id_obat."");
     return true;
  }
