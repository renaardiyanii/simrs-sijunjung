...
class Aplicares extends Secure_area {
	...
	public function botaddsemuaruangan()
	{
		...
		// Mengubah foreach untuk mendapatkan $index dan $v (value)
		foreach($bed_rs as $index => $v){
			
			// Menambahkan kondisi untuk hanya memproses jika indeks 53 atau lebih besar
			if ($index >= 53) {
				...
				// Logika pengecekan koderuang tetap di dalam
				if($v->koderuang != "110130"){
					$this->vclaim->post($url,$data,[],'https://apijkn.bpjs-kesehatan.go.id/');
				}

				// Delay 2 detik tetap dijalankan setelah setiap proses
				sleep(1);
			}
		}
	}
