  ...
  <div class="card-body">
    <form id="update_rencana_kontrol">
        <div class="mb-3 row">
            ...
            <label for="tgl" class="col-sm-4 col-form-label">Tgl.Rencana Kontrol</label>
            <div class="col-sm-8">
                <input type="date" class="form-control" id="tgl" name="tgl" onchange="pilihpolikontrol(this.value)">
            </div>
        </div>
        <div class="mb-3 row">
    ...
    <button id="btnsubmit" class="btn btn-primary" onclick="insert_rencana_kontrol()">Simpan</button>
  </div>
</div>
...
<script>
    ...
    function insert_rencana_kontrol()
    {
        let form = $("#update_rencana_kontrol").serialize();
        $.ajax({
            url: `<?= base_url('bpjs/rencanakontrol/insert_suratkontrol') ?>`,
            method:"post",
            data:form,
            beforeSend: function() {
            ...
            success: function(data) {
                if(data.metaData.code === '200'){
                    ...
                    window.location.href="<?= base_url('bpjs/rencanakontrol/cetak_surkon_spri/') ?>?data="+data.response.noSuratKontrol +'&nokartu='+data.response.noKartu;
                    return;
                }
