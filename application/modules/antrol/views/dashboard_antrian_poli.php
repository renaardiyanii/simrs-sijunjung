...
<body>
    ...
    <script>
        let namapoli = '';
        function getdata() {
            $.get('<?= base_url('antrol/antrol/get_data_dashboard_antrian_poli/' . $kodepoli) ?>', function (data, status) {
                // handle data
                // tambahkan berapa banyak dokter
                var banyakdokter = '';
                $("#poli").html(`ANTRIAN POLIKLINIK ${data[0].poli.toUpperCase()}`)
                data.map((e) => {
                    var listpasien = '';
                    e.pasien.map((i) => {
                        listpasien += `<li>${i.nama} (${i.norm})</li>`
                    });
                    banyakdokter += `
                    <div class="content-top col mx-2">
                        <center>
                            <div class="section">
                                <div class="sisa">${e.dokter}</div>
                                <div class="main-section">${e.pasiendilayani.nourut}</div>
                                <div class="sisa" style="min-height:80px">${e.pasiendilayani.nama} ${e.pasiendilayani.norm}</div>
