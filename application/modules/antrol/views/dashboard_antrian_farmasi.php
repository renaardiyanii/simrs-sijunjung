...
<body>
    <div class="container p-4">
        ...
        <div class="header d-flex justify-content-between align-items-center">
            ...
            <div>
                <span id="poli" style="font-size:34pt;">Antrian Farmasi</span>
            </div>
        ...
        <div class="row">
            <div class="col-md-8 content-top mb-4">
                <center>
                    <div class="sisa">Obat Siap diambil</div>
                    <div class="main-section" id="currentQueue">Loading...</div>
                    <div class="sisa" id="currentPatientName">Nama Pasien: -</div>
                </center>
            </div>
        ...
        <div class="row row-full">
            <div class="col-md-6">
                <div class="content-top">
                    <h4 class="text-center">Antrian Berikutnya</h4>
    ...
    <script>
        function getdata() {
            $.get('<?= base_url('antrol/get_data_dashboard_antrian_farmasi/') ?>', function (data, status) {
                // Update current queue
                if (data[0] && data[0].pasiendilayani) {
                    $("#currentQueue").html(`F-${String(data[0].pasiendilayani.nourut).padStart(3, '0')}`);
                    $("#currentPatientName").html(`Nama Pasien: ${data[0].pasiendilayani.nama || '-'}`);
                }
                ...
                data[0].pasien.forEach((item, index) => {
                    let formattedAntrian = `F-${String(item.nomorantrian).padStart(3, '0')}`;
                    let namaPasien = item.nama || '-';
                    if (index < 1) {
                        table1 += `<tr><td>${formattedAntrian}</td><td>${namaPasien}</td></tr>`;
                    } else if (index > 0 && index < 2) {
                        table2 += `<tr><td>${formattedAntrian}</td><td>${namaPasien}</td></tr>`;
                    }
                });
