...
class Tracer extends Secure_area {
	...
	{	
        ...
        if (empty($this->input->post('tgl_kunjungan'))) {
            ...
            $data['tgl'] = $this->input->post('tgl_kunjungan');
        }
    ...
    public function cetak($no_register) {        
        $ip_from = '192.168.114.19';//$this->input->ip_address();
      //  var_dump($ip_from);die();
        // $printer=$this->rjmpencarian->printer_tracer($ip_from)->row();   
    //    var_dump($printer->printer_tracer);die();
        if($no_register!=''){
            // if ($printer == '') {
                ...
                if($login_data->name=='' || $login_data->name==null){
                    ...
                    if ($row != '') {       
                        ...
                        $konten=
                            ...
                            <body id=\"body\" style='font-size:11px;'> 
                            <div id=\"print\">           
                            <hr>RSUD SIJUNJUNG <br/>
                            <br>
                        ...
                        //     document.body.innerHTML = restorepage;
                        // }
                        //         jsPrintSetup.setPrinter('$printer->printer_tracer'); 
                        //         jsPrintSetup.setOption('marginTop', 0);
                        //         jsPrintSetup.setOption('marginLeft', 0);
                        //         jsPrintSetup.setOption('marginRight', 0);
                        //         jsPrintSetup.setOption('headerStrLeft', '');
	...
	public function get_pasien()
    {
        ...
        if ($tgl_kunjungan == date('Y-m-d')) {
            $list = $this->rjmtracer->get_pasien($tgl_kunjungan);
            $data = array();
            $no = $_POST['start'];
            foreach ($list as $result) {
                ...
                } else $row[] = '<center><i class="fa fa-times"></i></center>';	
                if ( date("Y-m-d") == substr($result->tgl_daftar,0,10) ) 
                $jns_kunjungan="BARU";
                else $jns_kunjungan="LAMA";
                $row[] = $result->no_sep;
                $row[] = '<center><button class="btn btn-danger btn-sm delete-sep" onclick="cetak_tracer(\''.$result->no_register.'\',\''.$jns_kunjungan.'\')" style="margin-bottom:5px;"><i class="fa fa-print"></i> Cetak</button><a class="btn btn-success btn-sm" href="'.site_url("irj/tracer/cetak_sep").'/'.$result->no_sep.'" target="_blank">Cetak SEP</a></center>';
            ...
            $output = array(
                            "draw" => $_POST['draw'],
                            "recordsTotal" => $this->rjmtracer->count_all($tgl_kunjungan),
                            "recordsFiltered" => $this->rjmtracer->count_filtered($tgl_kunjungan),
                            "data" => $data,
                    );
    ...
    public function cetak_sep($no_sep = '')
    {
        ...
        $data['sep'] = $this->vclaim->get($param,$content_type);
        $json_data = json_decode($data['sep'])->metaData->message;
        $data['daftar_ulang_irj'] = $this->rjmregistrasi->get_detail_daful_by_no_sep($no_sep)->result();
        $data['get_cetakan_sep'] = $this->rjmregistrasi->get_cetakan_sep($no_sep)->row();
        // var_dump($data['get_cetakan_sep']);
        $cetakan = isset($data['get_cetakan_sep']->cetakan)?$data['get_cetakan_sep']->cetakan!=null?$data['get_cetakan_sep']->cetakan:null:null;
        if($cetakan){
            $update['cetakan'] = $cetakan+1;
            $this->rjmregistrasi->update_cetak_sep($no_sep,$update);
            $data['cetakan_ke'] = $update['cetakan'];
        }else{
            $data['cetakan_ke'] = 1;
            $update['cetakan'] = 1;
            $this->rjmregistrasi->update_cetak_sep($no_sep,$update);
        }
        // var_dump($data['daftar_ulang_irj']);

        $result = $json_data == 'Data Tidak Ditemukan'?redirect('irj/tracer'):$this->load->view('sep/view_sep',$data);
        echo $result;
