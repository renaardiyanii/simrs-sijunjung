...
//SetCellValue('B'.$rowCount, $row->no_medrec);
	...
	public function input_inacbg() {
		$data['title'] = 'Input Kode INA-CBG';
		$date = $this->input->post('date_days');

		if($date != '') {
			$data['date_title'] = date("d F Y", strtotime($date));
			$data['irj'] = $this->rjmmedrec->data_pasien_irj_inacbg($date)->result();
		} else {
			$data['date_title'] = date("d F Y", strtotime(date('Y-m-d')));
			$data['irj'] = $this->rjmmedrec->data_pasien_irj_inacbg(date('Y-m-d'))->result();
		}

		$this->load->view('irj/input_inacbg', $data);
	}

	public function input_inacbg_pasien($no_register) {
		$data['title'] = 'Input INA-CBG pasien Rawat Jalan - '.$no_register;
		$data['data_pasien'] = $this->rjmmedrec->get_data_pasien_irj($no_register)->row();
		$data['inacbg'] = $this->rjmmedrec->get_kode_inacbg()->result();
		$data['diagnosa'] = $this->rjmmedrec->get_diagnosa_inacbg($no_register)->result();
		$this->load->view('irj/data_pasien_inacbg', $data);
	}

	public function insert_inacbg() {
		$no_register = $this->input->post('no_register');

		$data['kode_inacbg'] = $this->input->post('kode_inacbg');
		$code = $this->rjmmedrec->update_kodeinacbg_pasien($data, $no_register);
		...
		if($code == true) {
			...
			redirect('irj/rjcmedrec/input_inacbg_pasien/'.$no_register);
		} else {
			$this->session->set_flashdata('pesan',
			"<div class='alert alert-error alert-dismissable'>
				<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
