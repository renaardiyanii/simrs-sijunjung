...
class Diagnosa extends Secure_area {
    ...
    public function insert_all()
    {
        ...
        $insert['xuser'] = $login_data->userid;
        if($this->Mdiagnosa->count_limit($data['no_register'])>30){
            $result_error = array(
        ...
        if(isset($data['jumlah_diagnosa']) && $data['jumlah_diagnosa']>0 ){
            foreach($data['jumlah_diagnosa'] as $val){
                $cek_utama = $this->Mdiagnosa->count_utama($data['no_register']);
                $insert['klasifikasi_diagnos'] = $cek_utama>0?'tambahan':'utama';
                $diagnosa = explode('@',$val);
                $insert['id_diagnosa'] = $diagnosa[0];
                $insert['diagnosa'] = $diagnosa[1];
                $insert['diagnosa_text'] = $diagnosa[2];
                $insert['no_register'] = $data['no_register'];
                if (substr($data['no_register'], 0,2) == 'RJ') {  
                    $exist = $this->db->from('diagnosa_pasien')->where('id_diagnosa',$insert['id_diagnosa'])->where('no_register',$data['no_register'])->get();
                }
                if (substr($data['no_register'], 0,2) == 'RI') {  
                    $exist = $this->db->from('diagnosa_iri')->where('id_diagnosa',$insert['id_diagnosa'])->where('no_register',$data['no_register'])->get();
                }
                $result = $exist->num_rows() == 0?$this->Mdiagnosa->insert($insert):null;
                
            }
        }
        echo json_encode(array('code'=>$result?200:404));
    ...
    public function insert() 
    {
        ...
        if ($cek_limit < 30) {
            ...
            if ($this->input->post('diagnosa') != '') { 
                ...
                $idDiagnosa = ''; 
                $namaDiagnosa = $this->input->post('cari_diag'); 
            } 
            // else {
