<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Rjmmedrec extends CI_Model{
		...
		function get_medrec_by_search($tgl_kunj,$ceklist_diag){
			...
			$select_tgl="";
			if ($tgl_kunj!='') {
				$select_tgl=" AND TO_CHAR(du.tgl_kunjungan,'YYYY-MM-DD')='$tgl_kunj'";
			}
...
// 		}
		function getdata_diagnosa_pulang_pasien_date($tgl_awal,$tgl_akhir,$id_poli){ 
			...
			return $this->db->query("
				SELECT a.tgl_kunjungan, a.jns_kunj, a.no_register, a.no_medrec, b.no_cm, a.diag_baru, a.diag_lama,j.nm_poli as poli, a.diagnosa as icd, b.nama, b.sex, a.umurrj as usia, a.cara_bayar, a.ket_pulang FROM
					...
					-- LEFT JOIN tni_pangkat k on b.pkt_id=k.pangka t_id
					WHERE  and a.tgl_kunjungan>='$tgl_awal' and a.tgl_kunjungan <='$tgl_akhir'
					$txtpoli
...
group by a.nm_poli");
		...
		function data_pasien_irj_inacbg($date) {
			return $this->db->query("SELECT 
				a.no_register,
				a.no_medrec,
				b.no_cm,
		...
		function get_data_pasien_irj($no_register) {
			return $this->db->query("SELECT 
				a.no_register,
				b.nama,
				b.no_cm,
				...
				c.nm_dokter,
				a.cara_bayar,
				(SELECT nmkontraktor FROM kontraktor WHERE a.id_kontraktor = id_kontraktor LIMIT 1) AS kontraktor,
				d.nm_poli,
				b.foto,
				a.kode_inacbg
			FROM 
				...
				LEFT JOIN poliklinik AS d ON a.id_poli = d.id_poli 
			WHERE 
				a.no_register = '$no_register'");
		}

		function get_kode_inacbg() {
			return $this->db->query("SELECT kd_inacbg, uraian FROM tarif_inacbg_new ORDER BY id_tarif_inacbg");
		}

		function update_kodeinacbg_pasien($data, $no_register) {
			$this->db->where('no_register', $no_register);
			$this->db->update('daftar_ulang_irj', $data);
			return true;
		}

		function update_kodeinacbg_pasien_ri($data, $no_register) {
			$this->db->where('no_ipd', $no_register);
			$this->db->update('pasien_iri', $data);
			return true;
		}

		function get_diagnosa_inacbg($no_register) {
			if(substr($no_register,0,2) == 'RI') {
				return $this->db->query("SELECT id_diagnosa, diagnosa, klasifikasi_diagnos FROM diagnosa_iri WHERE no_register = '$no_register'");
			} else {
				return $this->db->query("SELECT id_diagnosa, diagnosa, klasifikasi_diagnos FROM diagnosa_pasien WHERE no_register = '$no_register'");
			}
		}
	}
?>
