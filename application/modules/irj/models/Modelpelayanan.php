...
and du.status='0' order by du.tgl_kunjungan  asc");
		...
		function getdata_jenis_tindakan(){
			// var_dump($kelas);die();
			return $this->db->query("SELECT A.id_tindakan, A.total_tarif, A.tarif_iks, A.tarif_bpjs, A.kelas, B.idtindakan, B.nmtindakan, B.idpok1  FROM tarif_tindakan A, jenis_tindakan B WHERE A.id_tindakan=B.idtindakan AND A.kelas='III' order by idtindakan asc");
		}
