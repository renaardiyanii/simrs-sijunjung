...
class Mdiagnosa extends CI_Model{
 ...
 //    {                            
    ...
    function select2($keyword){
        ...
        // $query = $this->db->get();
        $query = $this->db->query("SELECT id_icd,nm_diagnosa 
        FROM icd1 
        WHERE id_icd LIKE '%$keyword%' AND deleted=0
        OR nm_diagnosa LIKE '%$keyword%' AND deleted=0
        OR diagnosa_indonesia LIKE '%$keyword%'AND deleted=0");
        return $query->result();
    }
    ...
    function update_bpjs_sep($noreg,$data){
        return $this->db->where('no_register',$noreg)->update('bpjs_sep',$data);
    }
