...
<script type='text/javascript'>
	$(document).ready(function () {	
        ...
        $('#id_obat').select2(); // Apply Select2 to your select element
	...
	<div class="row">
		<div class="card card-outline-info" style="width:97%;margin:0 auto">
			<div class="card-header">		
					...
					<h4  class="text-white" align="center">Laporan Waktu Tunggu Farmasi</h4>

			</div>
			<div class="card-block">
				<div class="row">
					<form action="<?php echo base_url();?>irj/rjclaporan/lap_jam_farmasi" method="post" accept-charset="utf-8">
						<div class="form-group row">
							...
							<div class="col-md-2">
								<a href="<?php echo site_url('irj/rjclaporan/excel_lap_waktu_tunggu_farmasi/'.$tgl.'/'.$tgl1);?>"><input type="button" class="btn 
						...
						<table class="table table-striped" id="dataTables-example">
							<thead>
								<tr>
									...
									<th>Poli</th>
									<th>Cara Bayar</th>
									<th>Waktu Mulai Resep</th>
									<th>Waktu Selesai Resep</th>
									<th>Detail Obat</th>
								</tr>
							...
							<?php 
								...
								foreach($data_item as $val){ 
									...
									<tr>
										...
										<td><?php echo $val->no_kartu ?></td>	
										<td><?php echo $val->no_sep ?></td>		
										<td><?php echo $val->nama ?></td>
										<td><?php echo $val->dokter ?></td>
										<td><?php echo $val->poli ?></td>
										<td><?php echo $val->cara_bayar ?></td>	
										<td><?php echo isset($val->waktu_resep_farmasi)?date('H:i',strtotime($val->waktu_resep_farmasi)):'-' ?></td>	
											<td><?php echo isset($val->waktu_selesai_farmasi)?date('H:i',strtotime($val->waktu_selesai_farmasi)):'-' ?></td>	
											<td>
										<?php
											foreach($data_resep as $resep){
											if($val->no_register == $resep->no_register){ ?>
												<?= 
													$resep->nama_obat.'('.$resep->qty.')'.$resep->vtot.'<br>';
												?>
												<?php }} ?>
										</td>							
									</tr>
