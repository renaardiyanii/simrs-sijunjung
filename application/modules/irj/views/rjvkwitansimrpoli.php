...
<script type='text/javascript'>
	...
	<div class="container-fluid">
		...
		<section class="content">
			...
			<tr id="penghambat">
				...
				<?php //if($data_pasien->cara_bayar=='KERJASAMA' || $data_pasien->cara_bayar=='BPJS'){?>
					<th>Dijamin Oleh</th>
					<td>:</td>
					<td><?php echo $data_pasien->cara_bayar//if($kontraktor!=''){echo $kontraktor->nmkontraktor;}?></td>
				<?php //}?>
		...
		<tbody>
			...
			<!-- <tr>
				...
				<td>:</td>
				<td colspan="2">
					<input type="text" class="form-control" name="note_diskon" id="note_diskon" placeholder="Catatan Diskon/Pengurangan">
			...
			<tr>
				...
				<td><div class="input-group">
						...
						<span class="input-group-btn">
						<button type="button" class="btn btn-primary" onclick="setTotal()">Hitung</button> 
					</span>
					</div>
				</td>
			</tr> -->
	...
	<br>
		<div class="form-inline row" align="left" style="margin-left:10px;">
			<div class="input-group">				
			<?php //echo form_open('irj/rjckwitansi/st_cetak_kwitansi_detail_kt',array('target'=>'_blank'));?>
            ...
            <?php //echo form_open('irj/rjckwitansi/st_cetak_kwitansi_detail_kt');?>
					...
					<input type="hidden" id="idtindakan" name="idtindakan">
					<span>
...
function submitPembayaran()
{
	...
	$.ajax({  
		url:"<?= base_url() ?>/irj/rjckwitansi/st_cetak_kwitansi_detail_kt",                         
		method:"POST",  
		data:data,   
		...
		{ 
			...
			function () {
				...
				// if(pembayaran === 'PIUTANG/IKS'){
				// 	window.location.href = '<?php //echo base_url() ?>'+"irj/rjckwitansi/kwitansi_all_rj";
				// 	return;
				// }
				window.open('<?php echo base_url() ?>'+'irj/rjckwitansi/cetak_kwitansi_kt/'+'<?= $no_register ?>','_blank');
				window.location.href = '<?php echo base_url() ?>'+"irj/rjckwitansi/kwitansi_detail";
				// window.location.href = '<?php echo base_url() ?>'+"irj/rjckwitansi/kwitansi_detail";
			}); 
		},
...
$(document).ready(function() {	    	
	...
	// added
	// $('#cetakKwitansi').on('submit', function(e){
...
$('#diskon_form').on('submit', function(e){  
        e.preventDefault();             
        document.getElementById("btn-diskon").innerHTML = '<i class="fa fa-spinner fa-spin" ></i> Menyimpan...';
        $.ajax({  
          url:"<?php echo base_url(); ?>irj/rjckwitansi/update_diskon_poli",                         
          method:"POST",  
          ...
          success: function(data)  
          { 
            document.getElementById("btn-diskon").innerHTML = 'Input';
            // $('#ulangModal').modal('hide'); 
            // document.getElementById("ulang_form").reset();
            swal({
									title: "Selesai",
