					...
					<h4 align="center">Laporan Kunjungan <?php echo $judul ?></h4></div>
						...
						<div class="card-body">
							...
							<?php }else{ ?>
								...
								<tr>
									<td rowspan="2">No</td>
									<td rowspan="2">POLIKLINIK/DOKTER</td>
									...
									<?php $i = 1; foreach($poli as $row2){ 
										...
										$array = json_decode(json_encode($diagnosa), True);
										$data_poli=array_column($array, 'id_poli');
									
										//Klo data tdk kosong, tampilkan
										if (in_array($row2->id_poli, $data_poli)) {	 ?>	
											<tr>
												<td colspan="11"><?php echo $row2->nm_poli ?></td>
											</tr>
											<?php 
												$total_umum = 0;
												$total_bpjs_pbi = 0;
												$total_bpjs_non_pbi = 0;
												$total_bpjs_mandiri = 0;
												$total_bukit_asam = 0;
												$total_telkom = 0;
												$total_inhealth = 0;
												$total_pln = 0;
												...
												foreach ($diagnosa as $row) { 
													if ($row->id_poli == $row2->id_poli) { 
														$total_umum += $row->umum;
														$total_bpjs_pbi += $row->bpjs_pbi;
														$total_bpjs_non_pbi += $row->bpjs_non_pbi;
														$total_bpjs_mandiri += $row->bpjs_mandiri;
														$total_bukit_asam += $row->bukit_asam;
														$total_telkom += $row->telkom;
														$total_inhealth += $row->inhealth;
														$total_pln += $row->pln;
											...
											<tr style="font-weight: bold;">
												...
												<td><?php echo $total_bpjs_pbi; ?></td>
												<td><?php echo $total_bpjs_non_pbi; ?></td>
												<td><?php echo $total_bpjs_mandiri; ?></td>
												<td><?php echo $total_bukit_asam; ?></td>
												<td><?php echo $total_telkom; ?></td>
												<td><?php echo $total_inhealth; ?></td>
												<td><?php echo $total_pln; ?></td>
