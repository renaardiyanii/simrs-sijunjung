  ...
  <section class="sheet padding-10mm">
    ...
    <div class="header">
      ...
      <div class="header-judul">
        <span style="font-size:15pt;">SURAT ELEGIBILITAS PESERTA <br>RSUD SIJUNJUNG</span>
      </div>
    </div>
    <div class="content">
      <table width="100%">
        <tr>
          ...
          <td>:</td>
          <td width="40%" style="font-weight:bold;font-size:15pt;"><?= isset($data) ? $data->response->noSep : '-'; ?></td>
          <td>PRB</td>
          <td>:</td>
          <td><?= isset($data) ? $daftarulang->prb : '-' ?></td>
        </tr>
        <tr>
          <td>Tgl.SEP</td>
          <td>:</td>
        ...
        <tr>
          <td>Nama Peserta</td>
          <td>:</td>
          <td><?= isset($data) ? $data->response->peserta->nama : '-'; ?></td>
          <td>Jns.Rawat</td>
          <td>:</td>
        ...
        <tr>
          <td>Tgl.Lahir</td>
          <td>:</td>
          <td><?= isset($data) ? $data->response->peserta->tglLahir . '&nbsp; Kelamin :' . ($data->response->peserta->kelamin == 'L' ? ' Laki - Laki' : ' Perempuan') : '-'; ?></td>
          <td>Jns. Kunjungan</td>
          <td>:</td>
          <td><?= isset($daftarulang) ? $daftarulang->jenis_kunjungan : '-' ?></td>
        </tr>
        <tr>
          <td>No.Telpon</td>
          <td>:</td>
          <td><?= isset($daftarulang) ? $daftarulang->notelp == "" ? '-' : $daftarulang->notelp : '-';  ?></td>
          <td>Poli Perujuk</td>
          <td>:</td>
          <td><?= '-' ?></td>
        </tr>
        ...
        <tr>
          ...
          <td>:</td>
          <td><?= $daftarulang->namafaskes ?></td>
        </tr>
        <tr>
          <td>Diagnosa Awal</td>
          <td>:</td>
          <td><?= $daftarulang->diagawal . ' - ' . $data->response->diagnosa ?></td>
          <td>Penjamin</td>
          <td>:</td>
          <td><?= isset($data) ? $data->response->klsRawat->penanggungJawab : '' ?></td>
        </tr>
        <tr>
          <td>Catatan</td>
          <td>:</td>
