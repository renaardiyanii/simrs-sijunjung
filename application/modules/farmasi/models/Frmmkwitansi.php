<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Frmmkwitansi extends CI_Model{
		...
		function get_list_kwitansi_rj_luar() {
			$date = date("Y-m-d");
			return $this->db->query("
			SELECT a.tgl_kunjungan,a.no_register,
			b.no_cm,b.nama,
			c.nm_poli FROM daftar_ulang_irj AS a,
			data_pasien AS b, poliklinik AS c 
			WHERE a.no_medrec = b.no_medrec 
			AND a.id_poli = c.id_poli 
			AND to_char(a.tgl_kunjungan, 'YYYY-MM-DD') = '$date'
			UNION ALL
			SELECT tgl_kunjungan,no_register,
			no_cm::varchar,nama,'PASIEN LUAR' as nm_poli
			from pasien_luar
		...
		function get_data_pasien_luar($no_register) {
			return $this->db->query("SELECT a.* from pasien_luar as a where a.no_register = '$no_register'");
		}
		...
		function get_data_pasien_by_noreg_new($no_register) {
			return $this->db->query("SELECT
			to_char( data_pasien.tgl_lahir, 'DD-MM-YYYY' ) AS tgl_lahir,
			data_pasien.no_cm AS no_cm,
			A.cara_bayar,
			a.idrg,
			a.bed,
			...
			data_pasien.sex AS sex,
			data_pasien.alamat AS alamat,
			data_pasien.goldarah AS goldarah,
		...
		WHERE
			...
			AND substr( A.no_register, 1, 2 ) <> 'PL' UNION
		SELECT
			pasien_luar.usia,
			'Pasien Luar' AS no_cm,
			C.cara_bayar,
			...
			C.no_medrec,
			C.no_register,
			pasien_luar.nama AS nama,
			'-' AS sex,
			pasien_luar.alamat AS alamat,
			'-' AS goldarah,
			C.tgl_kunjungan AS tgl_kunjungan,
			C.kelas,
			d.nm_dokter 
		FROM
			resep_pasien C,
			...
			resep_header d 
		WHERE
			C.no_resep = d.no_resep 
			AND C.no_register = pasien_luar.no_register 
			AND C.no_register = pasien_luar.no_register 
			AND C.no_register = '$no_register'");
		}
		...
		function get_data_permintaan_luar($no_register) {
			return $this->db->query("SELECT SUM(vtot + embalase::integer) as vtot_obat FROM resep_pasien WHERE no_register = '$no_register'");
		}

		function get_data_permintaan_rj_detail($no_register) {
			return $this->db->query("SELECT 
				*,(select kategori6 from master_obat where master_obat.id_obat = resep_pasien.item_obat::int) as kategori6
			FROM 
				resep_pasien 
			WHERE 
		...
		function get_embalase_pasien($no_register) {
			return $this->db->query("SELECT 
				embalase,
				COUNT(nama_obat) AS qty_obat
			FROM 
				resep_pasien 
			WHERE 
				no_register = '$no_register'
		...
		function update_status_cetak_kwitansi($no_register){
			...
			$this->db->query("UPDATE resep_pasien SET cetak_kwitansi='1', tgl_cetak = '$today' WHERE no_register='$no_register'");
			//$this->db->query("UPDATE resep_header SET diskon='$diskon' WHERE no_resep='$no_resep'");
			return true;
		}
