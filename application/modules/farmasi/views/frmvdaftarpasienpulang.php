...
<div class="row">
    <div class="col-lg-12 col-md-12">
        <h2>DAFTAR ANTRIAN PASIEN OBAT PULANG RAWAT INAP</h2>
...
$(document).ready(function() {
    ...
    $("#datepicker-proses").on('change', function() {
        $.ajax({
            url: `<?php echo site_url('farmasi/Frmcdaftar/get_daftar_resep_pasien_obat_pulang/'); ?>/${this.value}`,
    ...
    $.ajax({
        url: `<?php echo site_url('farmasi/Frmcdaftar/get_daftar_resep_pasien_obat_pulang/'.date('Y-m-d')); ?>`,
        success: function(response) {
...
function resep(noreg) {
    ...
    // ini untuk yg baru
    window.open(`<?= base_url('farmasi/frmcdaftar/permintaan_obat_petugas_pulang?no_register=') ?>${noreg}`, '_blank');
}
