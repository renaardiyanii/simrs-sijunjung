...
<script>
	...
	function excel() {
		...
		window.location.href = `<?php echo site_url('farmasi/Frmclaporan/laporan_pemakaian_obat_per_dokter_rajal'); ?>?tgl_pertama=${tgl_pertama}&tgl_kedua=${tgl_kedua}&idrg=${idrg}`
	}
	...
	function rekap() {
		...
		$.ajax({
			url: `<?php echo site_url('farmasi/Frmclaporan/laporan_pemakaian_obat_per_dokter_rajal'); ?>?tgl_pertama=${tgl_pertama}&tgl_kedua=${tgl_kedua}&json=true&idrg=${idrg}`,
			...
			success: function(response) {
				...
				response.map((e) => {
					...
					<tr>
						...
						<td>${e.namapoli}</td>
						<td>${e.cara_bayar}</td>
						<td>${ e.cara_bayar == 'BPJS'?'-':e.nmkontraktor}</td>
						<td>${e.namadokter}</td>
