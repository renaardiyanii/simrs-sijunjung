...
class Frmcpengembalian extends Secure_area
{
  ...
  function index($param='')
    ...
    $data['title'] = 'Meminjamkan dan Mengembalikan Persediaan Farmasi';
    ...
    $this->load->view('logistik_farmasi/Frmvpengembalian',$data);
  ...
  function insert_detail_pengembalian(){

    $login_data = $this->load->get_var("user_info");
    $data['user'] = $login_data->username;
    ...
    $return_id = $this->Frmmtransaksi->insert_detail_pengembalian($data);
    redirect('logistik_farmasi/Frmcpengembalian/form_detail_transaksi_pengembalian/'.$return_id);
        
  }


    function form_detail_transaksi_pengembalian($return_id=''){
      ...
      $data['produsen']=$this->Frmmtransaksi->master_produsen()->result();
      $data_detail_return=$this->Frmmtransaksi->get_return_barang($return_id)->result();
     
        foreach($data_detail_return as $row){
            $data['return_time'] = $row->tanggal;
            $data['id_retur'] = $row->return_id;
            $data['supplier_id'] = $row->id_supplier;                
      ...
      $data['select_gudang']=$this->Frmmtransaksi->cari_gudang()->result();
      $data['data_retur_item']=$this->Frmmtransaksi->getdata_retur_item($data['id_retur'])->result();

      $this->load->view('logistik_farmasi/Frmvdetailtransaksi_pengembalian',$data);
  }

  public function insert_pengembalian(){
    //  var_dump($this->input->post());die();
    $data['retur_id']=$this->input->post('retur_id');
    $data['id_obat']=$this->input->post('idobat');

    $data_obat=$this->Frmmtransaksi->getitem_obat($data['id_obat'])->result();
    foreach($data_obat as $row){
      $data['nm_obat']=$row->nm_obat;
    }
    ...
    $data['qty_besar']=$this->input->post('qtyb');
    $data['qty_kecil']=$this->input->post('qty_hide');
    $data['harga_besar']=$this->input->post('biaya_besar');
    $data['harga_kecil']=$this->input->post('biaya_kecil');
    $data['total']=$this->input->post('biaya_total');
    $data['satuanb']=$this->input->post('satuanb');
    $data['satuank']=$this->input->post('satuank');

    $this->Frmmtransaksi->insert_retur_item($data);
    

    redirect('logistik_farmasi/Frmcpengembalian/form_detail_transaksi_pengembalian/'.$data['retur_id']);


  }

  public function hapus_data_retur($retur_items_id='', $id_obat='', $id_retur=''){

    $id=$this->Frmmtransaksi->hapus_data_retur($retur_items_id, $id_obat,$id_retur);
    
    redirect('logistik_farmasi/Frmcpengembalian/form_detail_transaksi_pengembalian/'.$id_retur);
   
	}

  public function insert_verifikasi_pengembalian(){
  
    $retur_id = $this->input->post('retur_id_hidden');
    $login_data = $this->load->get_var("user_info");
    $group = $this->Frmmpo->getIdGudang($this->session->userid);
    $stock['id_gudang'] = $group->id_gudang;
    $stock['user'] = $login_data->name;
    $stock['retur']=$this->Frmmtransaksi->getdata_retur_item($retur_id)->result();
    $stock['header_retur']=$this->Frmmtransaksi->get_retur($retur_id)->row();
    $this->Frmmtransaksi->insert_stock_verifikasi_pengembalian($stock);
    redirect('logistik_farmasi/Frmcpengembalian');
  }
  ...
  public function get_data_retur(){
    ...
    $i=1;
    $hasil = $this->Frmmtransaksi->get_all_data_retur_date($date)->result();
    foreach ($hasil as $value) {

        $row2['no'] = $i++;
        $row2['no_faktur'] = $value->no_surat;
        $row2['tgl'] = date('d-m-Y',strtotime($value->tanggal));
        $row2['company_name'] = $value->pbf;
        $row2['status'] = $value->status;
        $row2['verif'] = isset($value->tgl_verif)?date('d-m-Y h:i:s',strtotime($value->tgl_verif)).' '.'('.$value->user_verif.')':'';
        $row2['aksi'] = " <a class=\"btn btn-warning btn-xs\" href=\"".base_url('logistik_farmasi/frmcpengembalian/form_detail_transaksi_pengembalian/'.$value->return_id)."\">Detail</a>";
  ...
  public function insert_stock_pembelian($id,$id_obat,$id_rec){
    ...
    public function get_biaya_obat()
	  {
		 $id_obat=$this->input->post('idobat');
		 $biaya=$this->Frmmtransaksi->get_data_inven_obat($id_obat)->result();
    //  var_dump($biaya);die();
    ...
    public function get_exp_obat()
	  {
		 $id_inven=$this->input->post('id_inven');
		 $biaya=$this->Frmmtransaksi->get_data_exp_obat($id_inven)->row();
    //  var_dump($biaya);die();
     echo json_encode($biaya);
