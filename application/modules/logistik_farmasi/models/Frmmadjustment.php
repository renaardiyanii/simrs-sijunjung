   ...
   function get_receivings($no_faktur) {
    ...
    $batch_no = $id_batch[1];
	  $where = "";
		...
		if ( $id_obat != ""){
			$where .= " AND i.id_obat = ".$id_obat." AND i.batch_no = '$batch_no'";
		}
	  return $this->db->query("
    SELECT i.batch_no, i.expire_date, i.id_gudang, i.id_inventory, i.id_obat, i.qty, m.nm_obat, g.nama_gudang, 
     

    (CASE WHEN i.expire_date::timestamp < NOW()
    THEN 1
    ELSE 0
    END )
    as expired
    ...
    function insert_selesai_adjustment($data1){
      ...
      //input histori
      $this->db->query("INSERT INTO history_obat (no_transaksi, id_obat, batch_no, created_date, created_by,keterangan, gudang1, stok_awal, adjustment, stok_akhir)
   VALUES (0, '".$data1['id_obat']."', '".$data1['batch_no']."', '".date('Y-m-d H:i:s')."','".$data1['username']."', 'Adjustment', '".$data1['id_gudang']."', '".$data1['qty']."', '".$data1['qty_adjustment']."', '".$data1['qty_real']."')");
      return true;
    }
    ...
    function data_gudang($id_inventory){
      return $this->db->query("SELECT b.id_inventory, b.id_gudang, a.nm_obat, a.id_obat, b.batch_no , b.qty, TO_CHAR(b.expire_date::timestamp,'YYYY-MM-dd') as expire_date from master_obat as a, gudang_inventory as b where a.id_obat = b.id_obat and b.id_inventory ='$id_inventory'");
    }
