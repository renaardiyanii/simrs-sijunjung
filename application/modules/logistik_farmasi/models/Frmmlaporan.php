...
/////////////////////////////////////////////////
	...
	public function get_penerimaan_obat($supplier="semua",$status="semua", $jenis_obat="semua", $tgl_awal=null, $tgl_akhir=null)
	{
		...
		public function get_rekap_obat_laporan($date1,$date2,$kode)
		{
			if($kode == '' || $kode == 'semua'){
				...
				.*,
				b.diterima_barang,
				( SELECT pbf FROM master_pbf WHERE b.supplier_id = master_pbf.ID ),
				c.subkelompok,
				( SELECT bentuk_sediaan FROM master_subkelompok_obat WHERE master_subkelompok_obat.kode = c.subkelompok )
			FROM
				receivings_items
				A LEFT JOIN receivings b ON A.receiving_id = b.receiving_id 
				LEFT JOIN master_obat c on c.id_obat = a.item_id
			WHERE
				'[$date1,$date2]' :: daterange @> CAST ( b.diterima_barang AS DATE )");
			}else{
				return $this->db->query("SELECT A
				.*,
				b.diterima_barang,
			...
			FROM
				...
				LEFT JOIN master_obat c on c.id_obat = a.item_id
			WHERE
				'[$date1,$date2]' :: daterange @> CAST ( b.diterima_barang AS DATE ) and c.subkelompok = '$kode'");
			}
	...
	public function master_kelompok_obat()
		...
		public function get_data_faktur_penerimaan_obat($ket,$verif,$tgl1,$tgl2)
		{
				if($ket == "semua" && $verif == 'semua'){
						return $this->db->query("SELECT
						(select count(receiving_id) from receivings_items where receivings.receiving_id = receivings_items.receiving_id) as jml,
						receiving_id,
						diterima_barang AS tgl_masuk,
						tgl_kontra_bon AS tgl_faktur,
						tgl_kontra_bon_do AS tgl_do,
						jatuh_tempo,
						( SELECT pbf FROM master_pbf WHERE supplier_id = master_pbf.ID ) AS distributor,
						no_faktur,
						no_do,
						total_price_awal,
						total_price 
					FROM
						receivings 
					WHERE
						diterima_barang >= '$tgl1' 
						AND diterima_barang <= '$tgl2' 
						order by receiving_id asc");

				}else if($ket == "semua" && $verif != 'semua'){
					...
					if($verif == 'penerima'){
						...
						WHERE
							diterima_barang >= '$tgl1' 
							AND diterima_barang <= '$tgl2' 
							AND tgl_verif_gudang IS NOT NULL 
							order by receiving_id asc");
					}
				}else if($ket != "semua" && $verif == 'semua'){
						return $this->db->query("SELECT
					...
					WHERE
						...
						AND diterima_barang <= '$tgl2' 
						and status = '$ket'
						AND tgl_verif_penerima IS NOT NULL order by receiving_id asc");
				}else if($ket != "semua" && $verif != 'semua'){
					if($verif == 'penerima'){
						return $this->db->query("SELECT
						(select count(receiving_id) from receivings_items where receivings.receiving_id = receivings_items.receiving_id) as jml,
		...
		public function get_data_detail_faktur_penerimaan_obat($jenis)
		{
			// var_dump($jenis);die();
				if($jenis == "semua"){
						return $this->db->query("SELECT * from rincian_penerimaan_obat");
				}else{
					return $this->db->query("SELECT * from rincian_penerimaan_obat where jenis_obat = '$jenis'");
				}	
		}

		public function get_data_stock_mutasi_gudang($tgl_awal,$tgl_akhir,$id_gd,$kel)
		{
			if($kel != 'semua'){
				return $this->db->query("SELECT
				e.satuank,
				...
				A.batch_no,
				( SELECT bentuk_sediaan FROM master_subkelompok_obat h WHERE h.kode = e.subkelompok ) AS subkel,
				( SELECT qty FROM gudang_inventory WHERE gudang_inventory.batch_no = A.batch_no AND gudang_inventory.id_gudang = $id_gd AND tgl_so IS NOT NULL AND qty >= 0 LIMIT 1 ) AS stock_awal,
				SUM (
					(
					CASE
					...
					SUM (
						...
						CASE
								...
								WHEN keterangan IN ( 'Adjusment_Tambah', 'Produksi Tambah', 'Penambahan Distribusi Langsung', 'Distribusi Masuk' ) THEN
								COALESCE ( A.adjustment, 0 ) + COALESCE ( A.produksi, 0 ) + COALESCE ( A.distribusi, 0 ) ELSE 0 
							END 
							) 
						) AS masuk_mutasi,
						...
						CASE
								...
								WHEN keterangan IN ( 'Adjusment_Kurang', 'Distribusi Keluar', 'Produksi Kurang', 'Pengurangan Distribusi Langsung' ) THEN
								COALESCE ( A.adjustment, 0 ) + COALESCE ( A.distribusi, 0 ) + COALESCE ( A.produksi, 0 ) + COALESCE ( A.distribusi, 0 ) ELSE 0 
							END 
							) AS keluar_mutasi,
							SUM ( CASE WHEN keterangan IN ( 'Transaksi Penjualan' ) THEN COALESCE ( A.penjualan, 0 ) ELSE 0 END ) AS keluar_pemakaian 
						FROM
							history_obat
							A LEFT JOIN master_obat e ON A.id_obat = e.id_obat 
						WHERE
							'[$tgl_awal,$tgl_akhir]' :: daterange @> CAST ( A.created_date AS DATE ) 
							AND gudang1 = $id_gd 
						GROUP BY
							A.batch_no,
							e.subkelompok,
		...
		function get_data_obat_all(){
        	return $this->db->query("SELECT A
					.id_obat,
					a.nm_obat,
					a.satuank,
					( SELECT nm_jenis FROM master_jenis_obat WHERE master_jenis_obat.ID = A.jns_obat LIMIT 1 ) AS jenis_obat,
						( SELECT nm_kategori_1 FROM master_kategori_obat WHERE master_kategori_obat.ID = A.kategori_obat limit 1) AS nama_kategori,
					( SELECT kode_rek FROM master_kategori_obat WHERE master_kategori_obat.ID = A.kategori_obat limit 1 ) AS kode_rek
		...
		function get_data_harga_obat($id_gd){
        	return $this->db->query("SELECT
				id_obat,
				batch_no,
				COALESCE ( hargajual, '0' ) AS harga 
			FROM
				gudang_inventory 
			WHERE
				id_gudang = $id_gd
		...
		function get_stok_laporan_stok_new($date,$gd){
			...
			return $this->db->query("
				...
				b.qty AS saldo_awal,
				'0' AS masuk,
				'0' :: DOUBLE PRECISION AS keluar,
				'0' :: DOUBLE PRECISION AS expire 
				FROM
					gudang_inventory_2024 b
					LEFT JOIN master_obat A ON A.id_obat = b.id_obat 
				WHERE
					to_char( b.tgl_so, 'YYYY-MM' ) = '$date' 
					AND b.id_gudang = $gd UNION
				SELECT
					d.nm_obat,
					d.satuank,
					( SELECT nm_jenis FROM master_jenis_obat WHERE master_jenis_obat.ID = d.jns_obat LIMIT 1 ) AS jenis_obat,
					kategori_obat,
					( SELECT kode_rek FROM master_kategori_obat WHERE master_kategori_obat.ID = d.kategori_obat LIMIT 1 ) AS kode_rek,
					d.id_obat,
					C.batch_no,
					...
					SUM (
						...
						CASE
								...
								WHEN keterangan IN ( 'Penerimaan Obat', 'Distribusi Masuk', 'Adjusment_Tambah' ) THEN
								COALESCE ( C.pembelian, 0 ) + COALESCE ( C.distribusi, 0 ) + COALESCE ( C.adjustment, 0 ) ELSE 0 
							END 
							) 
						) AS masuk,
						'0' :: DOUBLE PRECISION AS keluar,
					...
					FROM
						history_obat
						C LEFT JOIN master_obat d ON C.id_obat = d.id_obat 
					WHERE
						to_char( C.created_date, 'YYYY-MM' ) = '$date' 
						AND C.gudang1 = $gd 
					GROUP BY
						d.nm_obat,
						d.satuank,
					...
					SELECT
						f.nm_obat,
						f.satuank,
						( SELECT nm_jenis FROM master_jenis_obat WHERE master_jenis_obat.ID = f.jns_obat LIMIT 1 ) AS jenis_obat,
						kategori_obat,
						( SELECT kode_rek FROM master_kategori_obat WHERE master_kategori_obat.ID = f.kategori_obat LIMIT 1 ) AS kode_rek,
						f.id_obat,
						e.batch_no,
						to_char( e.expire_date, 'YYYY-MM-DD' ) AS expire_date,
						'0' harga_jual,
						'0' AS saldo_awal,
						'0' AS masuk,
						SUM (
							...
							CASE
									...
									WHEN keterangan IN ( 'Distribusi Keluar', 'Transaksi Penjualan', 'Adjusment_Kurang' ) THEN
									COALESCE ( e.pembelian, 0 ) + COALESCE ( e.distribusi, 0 ) + COALESCE ( e.adjustment, 0 ) ELSE 0 
								END 
								) 
							) AS keluar,
							'0' :: DOUBLE PRECISION AS expire 
						...
						WHERE
							to_char( e.created_date, 'YYYY-MM' ) = '$date' 
							AND e.gudang1 = $gd 
						GROUP BY
							f.nm_obat,
							f.satuank,
						...
						SELECT
							h.nm_obat,
							h.satuank,
							( SELECT nm_jenis FROM master_jenis_obat WHERE master_jenis_obat.ID = h.jns_obat LIMIT 1 ) AS jenis_obat,
							kategori_obat,
							( SELECT kode_rek FROM master_kategori_obat WHERE master_kategori_obat.ID = h.kategori_obat LIMIT 1 ) AS kode_rek,
							h.id_obat,
							G.batch_no,
							G.expire_date,
							G.hargajual AS harga_jual,
							'0' AS saldo_awal,
							'0' AS masuk,
							'0' :: DOUBLE PRECISION AS keluar,
							G.qty AS expire 
						FROM
							gudang_inventory_2024
							G LEFT JOIN master_obat h ON G.id_obat = h.id_obat 
						WHERE
							to_char( G.tgl_so, 'YYYY-MM' ) = '$date' 
							AND LEFT ( G.expire_date, 7 ) = '$date' 
							AND G.id_gudang = $gd
					ORDER BY
					id_obat ASC
			...
			// 		SELECT
			// 		b.id_obat,
			// 		SUM( b.distribusi) + SUM(b.penjualan ) AS keluar,
			// 		SUM( b.amprah ) AS masuk,
			// 		(
			// 		SELECT
			// 			SUM( qty ) 
			// 		FROM
			// 			gudang_inventory AS d
			// 		WHERE
			// 			id_gudang = $gd
			// 			AND LEFT ( d.tgl_so, 10 ) = '2023-12' 
			// 			AND d.id_obat = b.id_obat 
			// 		) AS sisa,
			// 		(
			// 		SELECT
			// 			SUM( stok_in ) 
			...
			// 		WHERE
			// 			c.tujuan = $gd
			// 			AND LEFT ( c.tanggal, 10 ) = '$date' 
			// 			AND c.id_obat = b.id_obat 
			// 		) AS masuk_pembelian 
			// 		FROM
			// 		history_obat AS b 
			// 		WHERE
			// 			b.gudang1 = $gd
			// 			AND LEFT ( b.created_date, 10 ) = '$date'
			// 		GROUP BY
			// 			b.id_obat
			// 	");
			// }else if($tampil == 'BLN'){
			...
			// 		WHERE
			// 			b.gudang1 = $gd
			// 			AND LEFT ( b.created_date, 4 ) = '$date'
			// 		GROUP BY
			// 			b.id_obat
		...
		function insert_cut_off_inven($tgl){
        	return $this->db->query("insert into gudang_inventory_2024 (id_inventory, tgl_so, id_obat, id_gudang, expire_date, hargajual, deleted, qty,batch_no)
 				select id_inventory, '$tgl', id_obat, id_gudang, expire_date, hargajual , deleted, qty,batch_no from gudang_inventory");
        }

		function get_kategori(){
        	return $this->db->query("
			select * from master_kategori_obat order by id asc");
        }

		public function get_distribusi($tgl,$tgl1,$ruang)
		{
			if($ruang != 'SEMUA'){
				return $this->db->query("select 
				a.id_amprah,
				...
				b.nm_obat,
				c.tgl_amprah,
				(select nama_gudang from master_gudang where a.id_gudang = master_gudang.id_gudang ) as gudang
				from distribusi a,
				master_obat b,header_amprah c where a.id_obat = b.id_obat
				and c.id_amprah = a.id_amprah AND to_char ( c.tgl_amprah,'YYYY-MM-DD' ) >= '$tgl' 
				AND to_char ( c.tgl_amprah,'YYYY-MM-DD' ) <= '$tgl1' and id_gudang = '$ruang'");
			}else{
				return $this->db->query("select 
				a.id_amprah,
				a.qty_req,
				...
				master_obat b,header_amprah c where a.id_obat = b.id_obat
				and c.id_amprah = a.id_amprah AND to_char ( c.tgl_amprah,'YYYY-MM-DD' ) >= '$tgl' 
				AND to_char ( c.tgl_amprah,'YYYY-MM-DD' ) <= '$tgl1' ");
			}
