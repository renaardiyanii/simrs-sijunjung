<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
  ...
  function get_data_stock($awal, $akhir,$id_gudang,$id_obat){
      ...
      CASE
          WHEN keterangan IN ( 'Transaksi Penjualan', 'Adjusment_Kurang', 'Distribusi Keluar','Produksi Kurang','Pengurangan Distribusi Langsung') THEN
          coalesce(A.penjualan,0) +coalesce(A.adjustment,0) + coalesce(A.distribusi,0) + coalesce(A.produksi,0)  ELSE 0 
        END AS keluar,
      CASE
      WHEN keterangan IN ( 'Penerimaan Obat', 'Adjusment_Tambah', 'Hapus Pembelian' ,'Distribusi Masuk', 'Produksi Tambah' , 'Penambahan Distribusi Langsung') THEN
        ...
        A.hargabeli ,
        A.expire_date,
        (select pbf from master_pbf where master_pbf.id = A.pbf :: int) as pbf,
        (select nama_gudang from master_gudang where master_gudang.id_gudang = A.gudang2 :: int) as gd,
        (select nama from data_pasien where data_pasien.no_medrec = A.no_medrec) as nama_pasien
      FROM
        history_obat
