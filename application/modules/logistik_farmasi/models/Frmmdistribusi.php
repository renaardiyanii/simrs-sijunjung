<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
  ...
  function get_amprah_detail_list($id){
    ...
    function get_amprah_detail_stock($ido, $idg){
		// var_dump($idg);die();
		return $this->db->query("SELECT batch_no, expire_date, qty, hargajual FROM gudang_inventory WHERE id_obat = $ido and id_gudang = $idg 
		and qty != 0 and deleted != 1 and  DATE_PART('year', expire_date::date) - DATE_PART('year', now()::date) >= 0
