...
class Frmmamprah extends CI_Model
{
    ...
    function getdata_amprah_by_role($id_gudang,$data, $url=null)
    {
        ...
        //     ")->result();
            ...
            SELECT a.id_amprah, a.tgl_amprah, a.gd_asal, a.gd_dituju, a.user,a.status,a.verif,d.nama_gudang as gudang_asal,m.nama_gudang as gudang_peminta,a.user_verif from header_amprah a
       LEFT JOIN master_gudang d ON A.gd_asal  = cast ( d.id_gudang as text )
	LEFT JOIN master_gudang M ON A.gd_dituju  = cast ( m.id_gudang as text ) 
      WHERE a.id_amprah > 0
    ...
    function getdata_amprah_by_role_dis($id_gudang,$data, $url=null)
    {
        // var_dump($id_gudang['id_gudang']);die();
        $userid = $this->session->userdata('userid');
        $login = $this->get_username($userid)->username;
      ...
      WHERE a.id_amprah > 0
      $where and gd_asal = '" . $id_gudang['id_gudang'] . "'
      ORDER BY a.tgl_amprah DESC
            ")->result();
    }
    ...
    //   ( SELECT qty_acc FROM distribusi b WHERE CAST ( A.id_amprah as int)  = b.id_amprah and cast(a.id_obat as int) = b.id_obat limit 1)  as qty_acc,(
    //     SELECT
    //       batch_no 
    //     FROM
    //       distribusi b 
    //     WHERE
    //       CAST ( A.id_amprah AS INT ) = b.id_amprah 
    //       AND CAST ( A.id_obat AS INT ) = b.id_obat 
    //       LIMIT 1 
    //     ) AS batch,
    //     (
    //     SELECT
    ...
    return $this->db->query("SELECT A.ID,
      A.id_amprah,
      A.id_obat,
      (select nm_obat from master_obat where master_obat.id_obat = A.id_obat) as nm_obat,
      A.satuank,
      A.qty_req,
      A.qty_acc,
      A.batch_no,
      A.expire_date
      from distribusi A where A.id_amprah = '$id'
      ")->result();
    ...
    function getsatuan_obat($id_obat){
      return $this->db->query("SELECT satuank FROM master_obat WHERE id_obat='".$id_obat."'")->row();
    }
    /*
    ...
    function insert_stock_verif($data)
    {
      ...
      $stock_akhir = $stock - $row->qty;
        ...
        $this->db->query("INSERT INTO history_obat (no_transaksi, id_obat, batch_no, created_date, keterangan, gudang1,gudang2, stok_awal, distribusi, stok_akhir,expire_date,created_by)
        VALUES ('".$data['header']->no_distribusi."', '".$row->id_obat."', '".$row->batch_no."', '".date('Y-m-d H:i:s')."', 
        'Pengurangan Distribusi Langsung', '".$data['header']->gd_asal."','".$data['header']->gd_dituju."', $stock , '".$row->qty."', $stock_akhir,'".$row->expire_date."','".$user_verif."')");
      ...
      if($cek_gd2->num_rows()){
        ...
        $this->db->query("INSERT INTO history_obat (no_transaksi, id_obat, batch_no, created_date, keterangan, gudang1,gudang2, stok_awal, distribusi, stok_akhir,expire_date,created_by)
          VALUES ('".$data['header']->no_distribusi."', '".$row->id_obat."', '".$row->batch_no."', '".date('Y-m-d H:i:s')."', 
         'Penambahan Distribusi Langsung', '".$data['header']->gd_dituju."','".$data['header']->gd_asal."', $jml , '".$row->qty."', $jml_akhir,'".$row->expire_date."','".$user_verif."')");
        ...
        $this->db->query("INSERT INTO history_obat (no_transaksi, id_obat, batch_no, created_date, keterangan, gudang1,gudang2, stok_awal, distribusi, stok_akhir,expire_date,created_by)
          VALUES ('".$data['header']->no_distribusi."', '".$row->id_obat."', '".$row->batch_no."', '".date('Y-m-d H:i:s')."', 
         'Penambahan Distribusi Langsung', '".$data['header']->gd_dituju."','".$data['header']->gd_asal."', $row->qty , '".$row->qty."', $row->qty,'".$row->expire_date."','".$user_verif."')");
  ...
  function check_stock_gd_asal2($id_gd,$id_obat,$batch){
    ...
    function get_obat_done_distribusi($id){
      ...
      return $this->db->query("SELECT A
          ...
          WHERE
            ...
            AND A.id_gudang_tujuan = b.id_gudang 
            AND id_amprah = '$id'
                ");

    }

    function insert_verifiksi_total($data_obat,$user){
      //  var_dump($data_obat);die();
          ...
          //input ke history obat di gudang yang minta amprah
          $this->db->query("INSERT INTO history_obat (no_transaksi, id_obat, batch_no, created_date, 
          keterangan, gudang1,gudang2, stok_awal, distribusi, stok_akhir, created_by,expire_date)
                VALUES ('0', '".$data["id_obat"]."', '".$data["batch_no"]."', '".date('Y-m-d H:i:s')."', 
          'Distribusi Masuk', '".$data["id_gudang"]."','".$data["id_gudang_tujuan"]."', '".$stock_asal_gd1."', '".$data["qty_acc"]."', '".$stock_akhir_gd1."', '".$user_verif."','".$expire_date."')");
          ...
          keterangan, gudang1,gudang2, stok_awal, distribusi, stok_akhir, created_by,expire_date)
                VALUES ('0', '".$data["id_obat"]."', '".$data["batch_no"]."', '".date('Y-m-d H:i:s')."', 
          'Distribusi Keluar', '".$data["id_gudang_tujuan"]."','".$data["id_gudang"]."', '".$stock_asal_gd2."', '".$data["qty_acc"]."', '".$stock_akhir_gd2."', '".$user_verif."','".$expire_date."')");
          // die();
          ...
          if ($check_stock > 0)			
            $this->db->query("UPDATE gudang_inventory SET qty = qty + '".$data["qty_acc"]."',expire_date = '".$expire_date."', hargajual  = '".$data["hargajual"]."'
            ...
            $this->db->query("UPDATE header_amprah SET verif = 1,user_verif = '".$user."' where id_amprah = '".$data["id_amprah"]."'");
    } }}
    ...
    {
        if ($this->db->query("insert into header_pinjam_farmasi(tgl_pinjam, tujuan_pinjam, \"user\")
							values(
									'" . date('Y-m-d') . "',
									'" . $data["vgd_dituju"] . "',
									'" . $data["user"] . "'
            ...
            $count = 0;
            $dataobat= array();
            $arr = array("id_header" => $id_pinjam);
            foreach ($json as &$value) {
                array_push($dataobat, array_merge($value, $arr));
                $count++;
            }

            $this->db->query("UPDATE header_pinjam_farmasi SET jmlh_pinjam = ".$count." WHERE id_header = '". $id_pinjam."'");

            foreach($dataobat as $value){
             

              $this->db->insert('pinjam_farmasi_items', $value);
    ...
    function get_tujuan_peminjaman(){

      return $this->db->query("SELECT * from master_pbf");
    }
    ...
    function getdata_pinjam_by_role($data, $url=null)
    {
        $userid = $this->session->userdata('userid');
        $login = $this->get_username($userid)->username;
        $where = "";
        ...
        if ($data["id_pinjam"] != "") {
            $where .= "WHERE id_header = " . $data["id_pinjam"];
        } else {
            if (($data["tgl0"] != "") && ($data["tgl1"] == "")) {
                $where .= " WHERE tgl_pinjam = '" . $data["tgl0"] . "'";
            }
            if (($data["tgl0"] != "") && ($data["tgl1"] != "")) {
                $where .= " WHERE tgl_pinjam BETWEEN ('" . $data["tgl0"] . "') AND ('" . $data["tgl1"] . "')";
            }
        }
           
            return $this->db->query("
            
            SELECT *,(select pbf from master_pbf where master_pbf.id = header_pinjam_farmasi.tujuan_pinjam) as tujuan from header_pinjam_farmasi
      $where 
      ORDER BY tgl_pinjam DESC
            ")->result();
    }

    function get_info_header_pinjam($id_pinjam)
    {
        return $this->db->query("SELECT *,(select pbf from master_pbf where master_pbf.id = header_pinjam_farmasi.tujuan_pinjam) as tujuan FROM header_pinjam_farmasi where id_header = $id_pinjam");
    }

    function get_detail_pinjam($id_pinjam)
    {
        return $this->db->query("SELECT * FROM pinjam_farmasi_items where id_header = $id_pinjam");
    }

    function get_info_amprah($id_amprah)
    {
        return $this->db->query("SELECT
        *,
        ( SELECT nama_gudang FROM master_gudang WHERE cast(master_gudang.id_gudang as text) = header_amprah.gd_asal ) AS untuk_depo,
        ( SELECT nama_gudang FROM master_gudang WHERE cast(master_gudang.id_gudang as text) = header_amprah.gd_dituju ) AS diminta_ke
      FROM
        header_amprah 
      WHERE
        id_amprah = $id_amprah");
