<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Frmmretur extends CI_Model{
		...
		function get_daftar_retur(){
		  ...
		  GROUP BY d.no_faktur , h.supplier_id,
			s.company_name,
			d.tgl_faktur,
			d.no_faktur,
			d.cara_bayar,
		...
		function get_data_retur_by_id($nofaktur){
			...
			return $this->db->query("SELECT p.*, r.qty AS quantity_retur, o.hargajual, SUM(g.qty) AS stok FROM po p INNER JOIN master_obat o ON o.id_obat = cast(p.item_id as integer) INNER JOIN gudang_inventory g ON g.id_obat = CAST(p.item_id AS INTEGER) AND g.batch_no = p.batch_no INNER JOIN \"do\" d ON d.id_po = p.id_po LEFT JOIN retur r ON r.no_faktur = d.no_faktur AND r.batch_no = p.batch_no WHERE d.no_faktur = '".$nofaktur."' AND g.id_gudang = 1 AND p.qty_beli > 0 GROUP BY
			p.batch_no,p.id_po,p.item_id,p.description,p.qty,p.satuank,p.qty_beli,p.expire_date,p.keterangan,p.user,p.hargabeli,p.diskon_item,p.harga_po,p.no_faktur,p.jml_kemasan,p.harga_item,p.satuan_item,p.diskon_persen,p.input_time,
			p.id, r.qty,o.hargajual");
		}
		...
		function get_data_retur_by_batch($batch_no, $id_obat, $id_gudang){
            ...
            return $this->db->query("SELECT g.id_obat AS item_id, o.nm_obat AS description, g.batch_no, g.qty AS quantity_purchased FROM gudang_inventory g INNER JOIN master_obat o ON o.id_obat = g.id_obat WHERE g.id_obat = '$id_obat' AND g.id_gudang = $id_gudang AND g.batch_no =  '$batch_no'
			");
		}
