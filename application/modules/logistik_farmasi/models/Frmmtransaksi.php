...
function update_verif_stock($id,$id_obat,$name){
  ...
  function cancel_pembelian($receiving_id){
   ...
   public function insert_stock_verifikasi_penerima($data)
   {
      ...
      $this->db->query("UPDATE receivings set verif_penerima = 1,pembulatan = '".$data['pembulatan']."',
      b_tambahan = '".$data['b_tambahan']."',total_price_awal = '".$data['total_price_awal']."',
      total_price='".$data['total_price']."',user_penerima='".$data['user']."',tgl_verif_penerima = now()
   ...
   public function insert_stock_verifikasi_gudang($data)
   {
      // var_dump($data);die();

      // update verif in table receiving
      $this->db->query("UPDATE receivings set verif = 1,user_gudang = '".$data['user']."',
      tgl_verif_gudang = now()
       where receiving_id = '".$data['header_receiving']->receiving_id."'");
      ...
      foreach ($data['receiving'] as $row) {
         ...
         if($cek_gd1->num_rows()){
            ...
            //cek harga 
            $cek_harga = $this->check_harga_gd_asal($data['id_gudang'],$row->batch_no,$row->item_id);
            if($cek_harga->row()->harga < $row->harga_jual){
               $this->db->query("UPDATE gudang_inventory SET qty = $stock + $row->quantity_purchased , ket = '".$data['header_receiving']->status."', hargajual =  '".$row->harga_jual."', expire_date = '".$row->expire_date."'
               where id_gudang = '".$data['id_gudang']."' and batch_no = '".$row->batch_no."' and id_obat = $row->item_id
                ");
            }else{
               $this->db->query("UPDATE gudang_inventory SET qty = $stock + $row->quantity_purchased , ket = '".$data['header_receiving']->status."', expire_date = '".$row->expire_date."'
               where id_gudang = '".$data['id_gudang']."' and batch_no = '".$row->batch_no."' and id_obat = $row->item_id
                ");
            }
   ...
   public function insert_stock_verifikasi_penerima_($data)
   {
      // var_dump($data['user_penerima']);die();

      // update verif in table receiving
      $this->db->query("UPDATE receivings set verif = 1,user_gudang = '".$data['user_penerima']."',
   ...
   function check_harga_gd_asal($id_gd,$batch,$id_obat){
		$query=$this->db->query("
      SELECT hargajual as harga
      from gudang_inventory 
      where id_gudang = '$id_gd' and batch_no = '$batch' and id_obat = $id_obat"); 
   ...
   function check_stock_gd_asal_pengembalian($id_inven){
		$query=$this->db->query("
      SELECT qty as jml
      from gudang_inventory 
   ...
   function insert_detail_pengembalian($data){
    ...
    function get_return_barang($return_id) {
      return $this->db->query("select *,(select pbf from master_pbf where return_barang.id_supplier = master_pbf.id ) as pbf from return_barang where return_id = $return_id");
   }

   function getdata_retur_item($id_retur){
      return $this->db->query("SELECT * FROM retur_items WHERE retur_id='$id_retur'");
   }

   function insert_retur_item($data){
      $this->db->insert('retur_items', $data);
      return true;
  }

  function hapus_data_retur($retur_item_id,$id_obat,$id_retur){ 
   return $this->db->query("DELETE FROM retur_items WHERE id_retur_items='$retur_item_id' AND id_obat='$id_obat' and retur_id = '$id_retur'");
   }

   function get_retur($retur_id) {
      $query = $this->db->get_where('return_barang', array('return_id'=>$retur_id));
      return $query;
   }

   public function insert_stock_verifikasi_pengembalian($data)
   {
      // var_dump($data);die();

      $this->db->query("UPDATE return_barang set verif = 1,user_verif = '".$data['user']."',
      tgl_verif = now()
       where return_id = '".$data['header_retur']->return_id."'");
      //cek stock gudang
      foreach ($data['retur'] as $row) {
         $cek_gd1 = $this->check_stock_gd_asal_pengembalian($row->id_inventory);
      
            if($cek_gd1->num_rows()){
               $stock = $cek_gd1->row()->jml;
               $stock_akhir = $stock - $row->qty_kecil;

               $this->db->query("INSERT INTO history_obat (no_transaksi, id_obat, batch_no, created_date,
               created_by, keterangan, gudang1, 
               stok_awal, retur_pembelian, stok_akhir,hargabeli,expire_date,pbf)
               VALUES ('".$data['header_retur']->no_surat."', '".$row->id_obat."', '".$row->batch_no."', '".date('Y-m-d H:i:s')."','".$data['user']."', 
               '".$data['header_retur']->status."', '".$data['id_gudang']."', $stock , 
               '".$row->qty_kecil."', $stock_akhir,
               '".$row->harga_kecil."','".$row->expire_date."','".$data['header_retur']->id_supplier."')");

               $this->db->query("UPDATE gudang_inventory SET qty = $stock - $row->qty_kecil 
      ...
      function get_all_data_retur_date($date){
         return $this->db->query("SELECT
         * ,
                ( SELECT SUM ( qty_kecil ) FROM retur_items WHERE retur_id = A.return_id GROUP BY retur_id ) 
                AS total ,
                   (select pbf from master_pbf where a.id_supplier = master_pbf.id)
             FROM
                return_barang AS A
             WHERE
                to_char(tanggal ::date ,'YYYY-MM-DD') ='$date'
             ORDER BY
                A.return_id DESC");
             }
      
      
             function get_data_inven_obat($id_obat){
               return $this->db->query("SELECT id_inventory,batch_no,expire_date,hargajual FROM gudang_inventory WHERE id_obat='".$id_obat."' and deleted = 0 and id_gudang ='1'");
            }

            function get_data_exp_obat($id_obat){
               return $this->db->query("SELECT id_inventory,batch_no,expire_date,hargajual FROM gudang_inventory WHERE id_inventory='".$id_obat."' and deleted = 0 and id_gudang ='1'");
            }

         function get_data_edit_receivings_item($id) {
            $query = $this->db->query("SELECT *,(select nm_produsen from master_produsen where master_produsen.id = receivings_items.prinsipal) as produsen from receivings_items where id_receivings_item = $id");
            return $query->row();
            }

            function edit_obat_penerimaan($id,$data){
               $this->db->where('id_receivings_item', $id);
               $this->db->update('receivings_items', $data); 
               return true;
            }

            function get_data_stok_cut($date){
               return $this->db->query("SELECT * FROM gudang_inventory_2024 where to_char(tgl_so,'YYYY-MM') = '$date'");
            }
