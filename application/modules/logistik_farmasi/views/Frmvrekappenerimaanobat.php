    ...
    <div class="col-lg-12 col-md-12">
        <div class="card">
            <div class="card-header">
                <form method="post" action="<?= base_url('logistik_farmasi/Frmclaporan/rekap_penerimaan_obat') ?>">
                    ...
                    <div class="row">    
                        ...
                        <div class="form-group col-md-3">
                            <select name="id_sub" id="filter" class="form-control">
                                <option value="semua" selected="">---- Pilih Semua ----</option>
                                <?php foreach ($subkelompok as $jns): ?>
                                <option value="<?= $jns->kode ?>"><?= $jns->bentuk_sediaan ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
    ...
    <div class="col-lg-12 col-md-12">
        ...
        <div class="card-block">
            <table class=" datatable table  table-striped">
                  ...
                  <tbody>
                    <?php $index=1; foreach ($rekap_penerimaan_obat as $val):?>
                      <tr>
                          <td><?= $index ?></td>
                          <td><?= $val->description ?></td>
                          <td><?= $val->pbf ?></td>
                          <td><?= $val->quantity_purchased ?></td>
                          <td><?= $val->satuanb ?></td>
                          <td><?= $val->biaya_total ?></td>
                          <?php 
                          $diskon = $val->biaya_kecil * $val->discount_percent;
                          $harga_diskon = $diskon / 100;
                          $jml_hna = $val->biaya_kecil - $harga_diskon;
                          $jml_ppn = 0.11 * $jml_hna;
                          
                          ?>
                          <td><?= $jml_ppn ?></td>
                          <td><?= $val->item_cost_price ?></td>
                         
                    <?php $index++; endforeach; ?>
                  </tbody>
                </table> 
                <a href="<?php echo site_url('logistik_farmasi/Frmclaporan/download_rekap_penerimaan_obat/'.$kode.'/'.$tgl_awal.'/'.$tgl_akhir);?>">
