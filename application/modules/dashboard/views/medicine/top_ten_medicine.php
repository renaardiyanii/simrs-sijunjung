...
?>
  <script>
   $('#opsi').change(function(){ 
        var value = $(this).val();
        opsi = value;
        window.location = '<?= base_url('/dashboard/top_ten_medicine/') ?>'+opsi;
    });
   ...
   $(document).ready(function(){
    ...
    const changeNamaBulan = (bulan) =>{
        ...
        $('.submitDiagnosa').click(function(){
            ...
            console.log(picker);
            window.location = '<?= base_url('/dashboard/top_ten_medicine/') ?>'+'/'+ picker;
        });
    ...
    //    get medicine kalo mau filter tambah opsi di sebelum where
       $.get( "<?= base_url('dashboard/get_medicine_ten/') ?>"+where, function( dataMentah ) {
           let dataMasak = JSON.parse(dataMentah);
           $.each(dataMasak, function( key, value ) {
                nmObat.push(value.nama_obat);
                jumlahObat.push(value.jumlah_obat); 
           });
           generateChart('myChart',`Daftar Obat Terbanyak ${where!=""?year +' '+convertBulan:new Date().getFullYear()}`,nmObat,jumlahObat,backgroundColor,borderColor);
