...
class Frmclaporan extends Secure_area {
    ...
    {
    	$data['suppliers'] = $this->Frmmlaporan->get_supplier()->result();
    	$data['supplier'] = "semua";
    	...
    	if ($this->input->post()) {
    		...
    		$data['supplier'] = $this->input->post('supplier')!="" ? $this->input->post('supplier') : "semua";
    	...
    	$data['penerimaan_obats'] = $this->Frmmlaporan->get_penerimaan_obat_by_faktur($data['supplier'],$data['status'],  $data['jenis_obat'], $data['tgl_awal'],$data['tgl_akhir'])->result();
    	$data['obats'] = $this->Frmmlaporan->get_penerimaan_obat($data['supplier'], $data['status'],$data['jenis_obat'], $data['tgl_awal'],$data['tgl_akhir'])->result();
    	$this->load->view('logistik_farmasi/Frmvpenerimaanobat', $data);
    }

    public function download_peneriamaan_obat($supplier="semua",$jenis="semua",$jenis_obat="semua",$tgl_awal=null,$tgl_akhir=null)
    {
    	////EXCEL
        $this->load->library('Excel');
	...
	public function data_min_stock($id)
  {
 		...
 		public function get_data_max()
		  ...
		  function download_rekap_penerimaan_obat($kode='',$tgl1,$tgl2){
			...
			//   $sheet->SetCellValue('G'.$rowCount, isset($row->item_cost_price)?$row->item_cost_price:'');
				...
				$sheet->SetCellValue('B'.$rowCount, $row->pbf);
				$sheet->SetCellValue('C'.$rowCount,	$row->quantity_purchased);
				$sheet->SetCellValue('D'.$rowCount, $row->satuank);
				$sheet->SetCellValue('E'.$rowCount,	$row->biaya_total);
				$sheet->SetCellValue('F'.$rowCount, 'test');
				$sheet->SetCellValue('G'.$rowCount, $row->item_cost_price);
				$rowCount++;
			}
		...
		public function lap_mutasi_obat()
		{
			...
			if ($this->input->post()) {
				...
				$data['lap_mutasi_obat'] = $this->Frmmlaporan->get_lap_mutasi_by_gudang( $data['bulan_old'],$data['tgl_awal'],$data['tgl_akhir'],$data['idgudang'],$data['kelompok'])->result(); 
			}
			$this->load->view('logistik_farmasi/Frmvlapmutasiobat', $data);
		}


		function download_lap_mutasi_obat($blnold='',$date1,$date2,$gd,$kel){
			
			$data = $this->Frmmlaporan->get_lap_mutasi_by_gudang($blnold,$date1,$date2,$gd,$kel)->result();
			// var_dump($data);die();
			$spreadsheet = new Spreadsheet();
			$sheet = $spreadsheet->getActiveSheet(); 
			...
			foreach($data as $row){
				...
				$sheet->SetCellValue('E'.$rowCount, $row->satuan_kecil);
				$sheet->SetCellValue('F'.$rowCount,	$row->hargabeli);
				$sheet->SetCellValue('G'.$rowCount, $row->stok_awal);
				$sheet->SetCellValue('H'.$rowCount, $row->masuk_gd);
				$sheet->SetCellValue('I'.$rowCount, $row->masuk_mutasi);
				$sheet->SetCellValue('J'.$rowCount, $row->total_masuk);
				$sheet->SetCellValue('K'.$rowCount, $row->keluar_gd);
				$sheet->SetCellValue('L'.$rowCount, $row->keluar_pemakaian);
				$sheet->SetCellValue('M'.$rowCount, $row->expire_date);
				$sheet->SetCellValue('N'.$rowCount, $row->total_keluar);
				$sheet->SetCellValue('O'.$rowCount, $row->sisa);
				$total_harga =  $row->hargabeli* $row->sisa;
