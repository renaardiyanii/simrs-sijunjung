...
function down_excel(){
    ...
    window.open('<?php echo site_url('rad/Radclaporan/excel_lap_bhp');?>/'+date1+'/'+date2, '_blank');      
}
...
function get_bln(){
    ...
    $.ajax({
        url: "<?php echo base_url();?>rad/radclaporan/lap_bhp_exe/"+date1+"/"+date2,
        beforeSend: function() {
        ...
        success: function(data) {
            ...
            if(data.data.length>0){
                data.data.map((e)=>{
                    html+=`
                    <tr>
                        <td><button class="btn btn-info btn-sm" data-toggle="modal" data-target="#detailModal" data-id=${e.id_pemeriksaan}><i class="fa fa-search"></i>Detail</button></td>
                        <td>${e.no}</td>
                        <td>${e.no_register}</td>
