...
class Klaim extends Secure_area {
    ...
    public function new_claim()
    {
    	...
    	$data_insert = array(
    		...
    		'nomor_kartu' => $this->input->post('no_bpjs'),	
		 	...
		 	'tgl_lahir' => $this->input->post('tgl_lahirr'),
	...
	function insert_klaim_inacbg()
	{
		$data_insert = array(
			'no_register' => $this->input->post('no_register'),
			'no_sep' => $this->input->post('no_sep'),
			'coder_nik' => $this->xuser->nik == '' || $this->xuser->nik == null ? '123123123123' : $this->xuser->nik,
			'status_klaim' => 1
		);
		$this->M_inacbg->insert_inacbg($data_insert);
	}

	function handle_procedure_inagrouper()
	{
		$procedure_inagrouper = $this->input->post('procedure_inagrouper');
		// buat penampung array , yg pertama buat nentuin lokasi , yg kedua buat delimeter +2 +3 dst.
		$tampung_procedure = [];
		$res_procedure_inagrouper = [];


		// cek jika procedure inagrouper null 
		if ($procedure_inagrouper != '' || $procedure_inagrouper != null) {
			// split string # jadi array 
			$procedure_inagrouper = explode('#', $procedure_inagrouper);

			// iterasi si hasil tadi
			foreach ($procedure_inagrouper as $val) {
				// jika belom ada , tambahkan ke penampung dan res
				if (!in_array($val, $tampung_procedure)) {
					array_push($tampung_procedure, $val);
					array_push($res_procedure_inagrouper, $val);
				}
				// jika ada , maka ambil index dari tampung_procedure , arahkan index ke 
				// res_procedure_inagrouper 
				else {
					// ambil index keberapa
					$key = array_search($val, $tampung_procedure);
					// masukan di res_procedure_inagrouper
					// cek , kalo kondisi si isi nya mengandung + ?
					if (strpos($res_procedure_inagrouper[$key], '+')) {
						// klao mengandung + , maka split string
						$str = explode('+', $res_procedure_inagrouper[$key]);
						// ambil yg string yg kedua , jadiin integer
						$counter = intval($str[1]) + 1;
						// satukan lagi string tadi , taro di tempatnya
						$res_procedure_inagrouper[$key] = $str[0] . '+' . strval($counter);
					}
					// kalo belom ada , maka tambahin +2
					else {
						$res_procedure_inagrouper[$key] = $res_procedure_inagrouper[$key] . '+2';
					}
				}
			}
		}
		return join('#', $res_procedure_inagrouper);
	}

	public function set_claim_data_new()
	{

		// yg pertama dia ambil cek klaim berdasarkan no sep
		$klaim_exist = $this->input->post('klaim_exist_db');
		// ambil kode tarif
		$kode_tarif = $this->M_inacbg->config_inacbg()->row()->kode_tarif;
		// var_dump($kode_tarif);die();
		// cek kondisi jika cek klaim gaada , insert ke inacbg
		$klaim_exist == '0' ? $this->insert_klaim_inacbg() : null;

		// jika noregister rj maka 
		// jenis_rawat , kelas_rawat , icu_indikator , icu los , ventilator diisi

		// rawatjalan = 2, rawat inap = 1
		$jenis_rawat = $this->input->post('rawat');
		$kelas_rawat = $jenis_rawat == '2' ? '3' : $this->input->post('kelas_rawat');
		$icu_indikator = $jenis_rawat == '2' ? '0' : ($this->input->post('icu_indikator') == '' ? '0' : $this->input->post('icu_indikator'));
		$icu_los = $jenis_rawat == '2' ? '' : ($icu_indikator == '1' ? $this->input->post('icu_los') : '');
		$ventilator_hour = $jenis_rawat == '2' ? '' : ($icu_indikator == '1' ? $this->input->post('ventilator_hour') : '');
		$upgrade_class_ind = $jenis_rawat == '2' ? "0" : ($this->input->post('upgrade_class_ind') == '' ? '0' : $this->input->post('upgrade_class_ind'));
		$upgrade_class_class = $jenis_rawat == '2' ? "" : ($upgrade_class_ind == '1' ? $this->input->post('upgrade_class_class') : '');
		$upgrade_class_los = $jenis_rawat == '2' ? "" : ($upgrade_class_ind == '1' ? $this->input->post('upgrade_class_los') : '');
		$add_payment_pct = $jenis_rawat == '2' ? "" : ($upgrade_class_ind == '1' ? ($this->input->post('add_payment_pct') == null ? '' : $this->input->post('add_payment_pct')) : '');


		// procedure inagrouper itu udah sesuai
		$procedure_inagrouper = $this->handle_procedure_inagrouper();

		$set_claim_data = array(
			'metadata' => array(
				'method' => 'set_claim_data',
				'nomor_sep' => $this->input->post('no_sep'), // ok
			),
			'data' => array(
				'nomor_sep' => $this->input->post('no_sep'), // ok
				'nomor_kartu' => $this->input->post('no_bpjs'), // ok
				'tgl_masuk' => $this->input->post('tgl_masuk'), // ok
				'tgl_pulang' => $this->input->post('tgl_pulang'), //ok
				"cara_masuk"=> "gp", 
				/**
				 * added -> 5.8 -> cara_masuk : 
				...
				**/
				'jenis_rawat' => $jenis_rawat, // ok
				'kelas_rawat' => $kelas_rawat, // ok 
				'adl_sub_acute' => '', // ok
				'adl_chronic' => '', //ok
				'icu_indikator' => $icu_indikator, // ok
				'icu_los' => $icu_los, // ok
				'ventilator_hour' => $ventilator_hour, // ok  
				'upgrade_class_ind' => $upgrade_class_ind, // ok 
				'upgrade_class_class' => $upgrade_class_class, // ok 
				'upgrade_class_los' => $upgrade_class_los, // ok 
				/**
				 * Added 5.8
				 * Parameter 
				...
				'add_payment_pct' => $add_payment_pct, // ok
				'birth_weight' => '0', // ok
				/**
				 * Added 5.8
				 * Parameter 
				...
				"sistole"=>120, 
				"diastole"=> 70,
				'discharge_status' => $this->input->post('discharge_status'), //ok
				'diagnosa' => $this->input->post('diagnosa'), // ok
				'procedure' => $this->input->post('procedure'), // ok
				// added
				'diagnosa_inagrouper' =>  $this->input->post('diagnosa'), // ok
				'procedure_inagrouper' => $procedure_inagrouper, // ok
				// end added
				'tarif_rs' => array(
					'prosedur_non_bedah' => $this->input->post('prosedur_non_bedah'), // ok
					'prosedur_bedah' => $this->input->post('prosedur_bedah'), // ok
					'konsultasi' => $this->input->post('konsultasi'), // ok
					'tenaga_ahli' => $this->input->post('tenaga_ahli'), // ok
					'keperawatan' => $this->input->post('keperawatan'), // ok
					'penunjang' => $this->input->post('penunjang'), // ok
					'radiologi' => $this->input->post('radiologi'), // ok
					'laboratorium' => $this->input->post('laboratorium'), // ok
					'pelayanan_darah' => $this->input->post('pelayanan_darah'), // ok
					'rehabilitasi' => $this->input->post('rehabilitasi'), // ok
					'kamar' => $this->input->post('kamar'), // ok
					'rawat_intensif' => $this->input->post('rawat_intensif'), // ok 
					'obat' => $this->input->post('obat'), // ok
					// added 
					'obat_kronis' => $this->input->post('obat_kronis'), // ok
					'obat_kemoterapi' => $this->input->post('obat_kemoterapi'), // ok
					// end added
					'alkes' => $this->input->post('alkes'), // ok
					'bmhp' => $this->input->post('bmhp'), // ok
					'sewa_alat' => $this->input->post('sewa_alat') // ok
				),
				// added	
				'pemulasaraan_jenazah' => $this->input->post('pemulasaraan_jenazah') == null ? '0' : $this->input->post('pemulasaraan_jenazah'), // ini belom
				'kantong_jenazah' => $this->input->post('kantong_jenazah') == null ? '0' : $this->input->post('kantong_jenazah'), // ini belom 
				'peti_jenazah' => $this->input->post('peti_jenazah') == null ? '0' : $this->input->post('peti_jenazah'), // ini belom
				'plastik_erat' => $this->input->post('plastik_erat') == null ? '0' : $this->input->post('plastik_erat'), // ini belom
				'desinfektan_jenazah' => $this->input->post('desinfektan_jenazah') == null ? '0' : $this->input->post('desinfektan_jenazah'), // ini belom
				'mobil_jenazah' => $this->input->post('mobil_jenazah') == null ? '0' : $this->input->post('mobil_jenazah'), // ini belom
				'desinfektan_mobil_jenazah' => $this->input->post('desinfektan_mobil_jenazah') == null ? '0' : $this->input->post('desinfektan_mobil_jenazah'), // ini belom
				'covid19_status_cd' => $this->input->post('covid19_status_cd') == null ? '0' : $this->input->post(''), // ini belom
				'nomor_kartu_t' => $this->input->post('nomor_kartu_t') == null ? '0' : $this->input->post('nomor_kartu_t'), // ini belom
				'episodes' => $this->input->post('episodes') == null ? '0' : $this->input->post('episodes'), // ini belom
				'covid19_cc_ind' => $this->input->post('covid19_cc_ind') == null ? '0' : $this->input->post('covid19_cc_ind'), // ini belom
				'covid19_rs_darurat_ind' => $this->input->post('covid19_rs_darurat_ind') == null ? '0' : $this->input->post('covid19_rs_darurat_ind'), // ini belom
				'covid19_co_insidense_ind' => $this->input->post('covid19_co_insidense_ind') == null ? '0' : $this->input->post('covid19_co_insidense_ind'), // ini belom
				'covid19_penunjang_pengurang' => [
					'lab_asam_laktat' => $this->input->post('lab_asam_laktat') == null ? '0' : $this->input->post('lab_asam_laktat'), // ini belom
					'lab_procalcitonin' => $this->input->post('lab_procalcitonin') == null ? '0' : $this->input->post('lab_procalcitonin'), // ini belom
					'lab_crp' => $this->input->post('lab_crp') == null ? '0' : $this->input->post('lab_crp'), // ini belom
					'lab_kultur' => $this->input->post('lab_kultur') == null ? '0' : $this->input->post('lab_kultur'), // ini belom
					'lab_d_dimer' => $this->input->post('lab_d_dimer') == null ? '0' : $this->input->post('lab_d_dimer'), // ini belom
					'lab_pt' => $this->input->post('lab_pt') == null ? '0' : $this->input->post('lab_pt'), // ini belom
					'lab_aptt' => $this->input->post('lab_aptt') == null ? '0' : $this->input->post('lab_aptt'), // ini belom
					'lab_waktu_pendarahan' => $this->input->post('lab_waktu_pendarahan') == null ? '0' : $this->input->post('lab_waktu_pendarahan'), // ini belom
					'lab_anti_hiv' => $this->input->post('lab_anti_hiv') == null ? '0' : $this->input->post('lab_anti_hiv'), // ini belom
					'lab_analisa_gas' => $this->input->post('lab_analisa_gas') == null ? '0' : $this->input->post('lab_analisa_gas'), // ini belom
					'lab_albumin' => $this->input->post('lab_albumin') == null ? '0' : $this->input->post('lab_albumin'), // ini belom
					'rad_thorax_ap_pa' => $this->input->post('rad_thorax_ap_pa') == null ? '0' : $this->input->post('rad_thorax_ap_pa'), // ini belom

				],
				'terapi_konvalesen' => $this->input->post('terapi_konvalesen') == null ? '0' : $this->input->post('terapi_konvalesen'), // ini belom
				'akses_naat' => $this->input->post('akses_naat') == null ? '0' : $this->input->post('akses_naat'), // ini belom
				'isoman_ind' => $this->input->post('isoman_ind') == null ? '0' : $this->input->post('isoman_ind'), // ini belom
				'bayi_lahir_status_cd' => $this->input->post('bayi_lahir_status_cd') == null ? '0' : $this->input->post('bayi_lahir_status_cd'), // ini belom
				// end added


				'tarif_poli_eks' => '0', // tidak ada poli eksekutif
				'nama_dokter' => $this->input->post('nama_dokter'), // ok
				'kode_tarif' => $kode_tarif, // ok
				'payor_id' => explode('#', $this->input->post('payor'))[0], //ok		
				'payor_cd' => explode('#', $this->input->post('payor'))[1], //ok
				'cob_cd' => $this->input->post('cob_cd'), //ok
				'coder_nik' => $this->xuser->nik == '' || $this->xuser->nik == null ? '123123123123' : $this->xuser->nik, //ok
			)
		);
		$data_setklaim = json_encode($set_claim_data);
		// print_r($data_setklaim);die();
		$response_setclaim = $this->inacbg->web_service($data_setklaim);
		// var_dump($response_setclaim);die();
		...
		if ($response_setclaim_array->metadata->code == '200') {
			...
			$data_update_setclaim = array(
				...
				'procedure' => $this->input->post('procedure'),
				'tarif_poli_eks' => '0', // tidak ada poli eksekutif		
				'payor_id' => explode('#', $this->input->post('payor'))[0],
				'payor_cd' => explode('#', $this->input->post('payor'))[1],
			...
			// var_dump($response_grouper1_array);die();
			// die();
