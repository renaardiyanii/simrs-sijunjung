...
class Pasien extends Secure_area {
  ...
  public function index_backup(){
    $data = [
      'title'=>'Coding / Grouping',
      'kelompok_tarif'=>$this->M_inacbg->kelompok_tarif()
    ];
  ...
  // added @aldi
  // pembaruan & improve 16/12/2022
  // eklaim
  function get_autocomplete_new()
  {
    if (isset($_GET['term'])){
      ...
      // 3. Nama Pasien == OK
      // bedakan kategori 
      // jika tidak mengandung angka  = nama
      if (!preg_match('#[0-9]#', $q)) {
        $category = '3';
      }
      // jika string awal 0901r004 -> no sep
      else if(substr($q,0,8) == '0311R001'){
        $category = '2';
      }
      // kalo bukan berarti norm
      else{
        $category = '1';
      }
      // var_dump($category);die();
      $this->M_pasien->get_autocomplete($category,strtoupper($q));
    }
  }

  public function get_pelayanan_new()
  {
    $line  = array();
    $line2 = array();
    $row2  = array();
    ...
    foreach ($data_klaim as $value) {
      ...
      $row2['no_sep'] = isset($value->no_sep)?$value->no_sep:"";
      $row2['jaminan'] = isset($value->jaminan)?$value->jaminan:'';  
  ...
  {            
      $delete_if_exist = $this->M_pasien->delete_inacbg_tarif_rs($no_register);                             
      ...
      // $list_tindakan_pasien = $this->rimtindakan->get_list_sumtindakan_pasien_by_no_ipd($no_ipd); 
        ...
        foreach ($data_tindakan as $row) {
          if($row->idkel_inacbg == 1){
            ...
            $result['tarif_rehabilitasi']+=$row->vtot; //16
          } else if($row->idkel_inacbg == 17){
            $result['tarif_obat_kronis']+=$row->vtot; //17
          } else if($row->idkel_inacbg == 18){
            $result['tarif_obat_kemoterapi']+=$row->vtot; //18
          } else {
        ...
        // patologi anatomi tidak ada di rsomh
        // $data_pa=$this->M_pasien->getdata_pa_pasien($no_register)->result();           
        // foreach ($data_pa as $row1) {
        //   if($row1->idkel_inacbg == 1){
        //     $result['tarif_tenaga_ahli']+=$row1->vtot; //1
        ...
        $data_resep=$this->M_pasien->getdata_resep_pasien_ranap($no_register)->result();         
        foreach ($data_resep as $row1) {
  ...
  // penambahan function select 2 inacbg
  function select2_diagnosa_inacbg()
  {
    $keyword_explode = [];
    $keyword_explode = explode(" ", trim($_GET['q']));
    if (isset($_GET['q'])) {
      ...
      if ($response == '' || $response == null) {
        ...
        if (isset($check_result->metadata->code) && $check_result->metadata->code == '200') {
          ...
          if ($result_object == "EMPTY") {
            ...
            if (sizeof($keyword_explode) > 1) {
              for ($i = 1; $i < sizeof($keyword_explode); $i++) {
                for ($j = 0; $j < sizeof($row_set[$i - 1]); $j++) {
                  if (stripos($row_set[$i - 1][$j]['text'], $keyword_explode[$i]) !== FALSE) {
                    $row_set[$i][] = $row_set[$i - 1][$j];
                  }
                }
              }
              echo json_encode($row_set[sizeof($keyword_explode) - 1]);
            } else {
              echo json_encode($row_set[0]);
            }
          }
  ...
  // penambahan function insert diagnosa inacbg
  public function insert_diagnosa_inacbg()
  {
  ...
  // penambahan function delete diagnosa inacbg
  public function delete_diagnosa_inacbg()
  {
