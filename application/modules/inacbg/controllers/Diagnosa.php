...
class Diagnosa extends Secure_area {
    ...
    public function get_diagnosa_inacbg_new()
    {
        ...
        foreach ($data_diagnosa as $diagnosa) {
            ...
            // added for search diagnosa inagrouper
            $requestinagrouper = json_encode([
                'metadata'=>[
            ...
            $res = $this->inacbg->web_service($requestinagrouper);
            if ($res == '' || $res == null) { 
                $diagnosa_inagrouper = '-';             
            } else {            
                $check_result = json_decode($res);                                 
                if (isset($check_result->metadata->code) && $check_result->metadata->code == '200') {
                    ...
                    if ($result_object == "EMPTY") {
                        $diagnosa_inagrouper = '<center><span style="color: #dd4b39;">Tidak Valid</span></center>';
                    } else {
                        foreach ($result_object as $row) {
                            if($row->code == $diagnosa->id_diagnosa) {
                                $diagnosa_inagrouper = '<center><span style="color: #00a65a;">Valid</span></center>';
                            } else {
                                $diagnosa_inagrouper = '<center><span style="color: #dd4b39;">Tidak Valid</span></center>';
                            }
                        }
                    }                   
                } else $diagnosa_inagrouper = '<center><span style="color: #dd4b39;">Tidak Valid</span></center>';
            }  
            // end added
            $request = array(
                'metadata'=>array(
            ...
            $row[] = '<table class="table borderless" cellpadding="1"><tbody><tr><td><span class="code">'.$diagnosa->id_diagnosa.'</span></td><td>'.$diagnosa->diagnosa.'</td></tr></tbody></table>'; 
            $row[] = $diagnosa->diagnosa_text;
            $row[] = $diagnosa_inagrouper;
