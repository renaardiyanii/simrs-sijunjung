...
class M_pasien extends CI_Model {   
    ...
    {        
        ...
        // $query = $this->db->query("
        // SELECT a.no_cm,a.nama,b.no_register,b.no_sep,b.tgl_kunjungan,CAST(b.tgl_pulang AS VARCHAR),c.payor_cd AS jaminan FROM data_pasien AS a INNER JOIN daftar_ulang_irj AS b ON a.no_medrec = b.no_medrec LEFT JOIN inacbg_klaim AS c ON b.no_register = c.no_register WHERE a.no_cm = '$no_cm' AND b.cara_bayar = 'BPJS' GROUP BY tgl_kunjungan,no_cm,
        // nama,b.no_register,b.no_sep,b.tgl_pulang,c.payor_cd
        //             UNION ALL
        //             SELECT a.no_cm,a.nama,b.no_ipd AS no_register,b.no_sep,b.tgl_masuk AS tgl_kunjungan,b.tgl_keluar AS tgl_pulang,c.payor_cd AS jaminan FROM data_pasien AS a INNER JOIN pasien_iri AS b ON a.no_medrec = b.no_medrec LEFT JOIN inacbg_klaim AS c ON b.no_ipd = c.no_register WHERE a.no_cm = '$no_cm' AND b.carabayar = 'BPJS'
        //             ORDER BY tgl_kunjungan DESC
        // ");     
        ...
        $query = $this->db->query("
        SELECT a.no_cm, a.nama, b.no_register, b.no_sep, b.tgl_kunjungan, CAST(b.tgl_pulang AS VARCHAR), c.payor_cd AS jaminan
        FROM data_pasien AS a
        INNER JOIN (
            SELECT no_medrec, no_register, no_sep, tgl_kunjungan, cast(tgl_pulang as varchar)
            FROM daftar_ulang_irj
            WHERE cara_bayar = 'BPJS'
            UNION
            SELECT no_medrec, no_ipd AS no_register, no_sep, tgl_masuk AS tgl_kunjungan, tgl_keluar AS tgl_pulang
            FROM pasien_iri
            WHERE carabayar = 'BPJS'
        ) b ON a.no_medrec = b.no_medrec
        LEFT JOIN inacbg_klaim AS c ON b.no_register = c.no_register
        WHERE a.no_cm = '$no_cm'
        GROUP BY tgl_kunjungan, a.no_cm, a.nama, b.no_register, b.no_sep, b.tgl_pulang, c.payor_cd
        ORDER BY tgl_kunjungan DESC;
        ");
        return $query->result();
    } 
    ...
    /**
     * filter query untuk pencarian auto complete cari no rm / no sep / nama
     * category 1 => no rm
     * category 2 => no sep
     * category 3 => nama
     */
    ...
    {
        if($category == '1'){
            $query = $this->db->query("SELECT no_cm,nama,sex,tgl_lahir,no_kartu FROM data_pasien WHERE no_cm LIKE '%$q%' LIMIT 5");
        }elseif($category == '2')
        {
            $query = $this->db->query("
            SELECT data_pasien.no_cm,data_pasien.nama,data_pasien.sex,data_pasien.tgl_lahir,data_pasien.no_kartu FROM bpjs_sep 
            INNER JOIN data_pasien 
            ON data_pasien.no_medrec = bpjs_sep.no_medrec 
            WHERE bpjs_sep.no_sep like '$q%'
            AND data_pasien.no_medrec  IS NOT NULL
            limit 5");
        }elseif($category == '3'){
            $query = $this->db->query("SELECT no_cm,nama,sex,tgl_lahir,no_kartu FROM data_pasien WHERE nama LIKE '%$q%' LIMIT 5");
        }else{
            $query = $this->db->query("SELECT no_cm,nama,sex,tgl_lahir,no_kartu FROM data_pasien WHERE nama LIKE '%$q%' LIMIT 5");
        }
        if($query->num_rows() > 0){
    ...
    function show_pelayanan_irj($no_register) {
        ...
        $this->db->SELECT("daftar_ulang_irj.no_sep,data_pasien.no_kartu,TO_CHAR(daftar_ulang_irj.tgl_kunjungan, ,'YYYY-MM-dd HH24:MM:SS') as tgl_masuk,TO_CHAR(daftar_ulang_irj.tgl_pulang, ,'Mon-dd-YYYY HH24:MM:SS') as tgl_pulang,daftar_ulang_irj.id_dokter,data_pasien.no_cm,data_pasien.nama,data_pasien.tgl_lahir,data_pasien.jenis_identitas,data_pasien.no_identitas,data_pasien.sex as gender,daftar_ulang_irj.no_medrec,daftar_ulang_irj.id_poli,daftar_ulang_irj.ket_pulang,data_pasien.kelas_bpjs,data_dokter.nm_dokter");   
        $this->db->where('no_register', $no_register);
        $query = $this->db->get();
        return $query->row();
    } 
    ...
    function show_pelayanan_iri($no_register) {
        ...
        $this->db->JOIN('data_dokter', 'a.id_dokter = data_dokter.id_dokter', 'inner');          
        $this->db->SELECT("a.no_sep,a.vtot_rad,a.vtot_obat,a.vtot_lab,a.biaya_alkes,b.no_kartu,TO_CHAR(a.tgldaftarri, 'YYYY-MM-dd HH24:MM:SS') as tgl_masuk,jam_keluar as tgl_pulang,a.id_dokter,b.no_cm,a.klsiri,b.nama,b.tgl_lahir,b.jenis_identitas,b.no_identitas,b.sex as gender,a.keadaanpulang,b.kelas_bpjs,data_dokter.nm_dokter");   
    ...
    function getdata_lab_pasien_rj($no_register){
        return $this->db->query("SELECT a.jenis_tindakan,a.vtot,b.idkel_inacbg FROM pemeriksaan_laboratorium a LEFT JOIN jenis_tindakan b ON a.id_tindakan=b.idtindakan LEFT JOIN daftar_ulang_irj c ON a.no_register=c.no_register WHERE c.no_register='$no_register' AND a.no_lab IS NOT NULL");
    }
    
    function getdata_rad_pasien_rj($no_register){
        return $this->db->query("SELECT a.jenis_tindakan,a.vtot,b.idkel_inacbg FROM pemeriksaan_radiologi a LEFT JOIN jenis_tindakan b ON a.id_tindakan=b.idtindakan LEFT JOIN daftar_ulang_irj c ON a.no_register=c.no_register WHERE c.no_register='$no_register' AND a.no_rad IS NOT NULL");
    }
    ...
    function getdata_resep_pasien_ranap($no_register){
        return $this->db->query("SELECT a.nama_obat,a.vtot::integer + a.embalase::integer as vtot,a.no_register FROM resep_pasien a WHERE a.no_register='$no_register' AND a.cara_bayar='BPJS'");
    }
    ...
    function getdata_ruangan_pasien_ri($no_register){
        ...
        SELECT
                a.idrg,a.kelas,
                CASE
                    WHEN (tglmasukrg::timestamp - tglkeluarrg::timestamp) = INTERVAL '0 days' THEN 1
                    ELSE EXTRACT(EPOCH FROM (tglkeluarrg::timestamp - tglmasukrg::timestamp)) / 86400
                END AS lama_rawat,
                c.tarif_bpjs,
                d.nmruang,
                CASE
                    WHEN (tglmasukrg::timestamp - tglkeluarrg::timestamp) = INTERVAL '0 days' THEN c.tarif_bpjs
                    ELSE EXTRACT(EPOCH FROM (tglkeluarrg::timestamp - tglmasukrg::timestamp)) / 86400 * c.tarif_bpjs
                END AS vtot
            FROM
                ruang_iri AS a
            ...
            WHERE
                a.no_ipd='$no_register' AND
                c.id_tindakan LIKE '1%'
            ORDER BY
                tglmasukrg ASC
    ");
    }
