	...
	<div class="row">				
		<div class="card" style="width:97%;margin:0 auto">
			<div class="card-header">
				<h3 class="card-title">Daftar Pasien <?= $nm_perusahaan ?></h3>			
			</div>
					...
					<table id="tabel_kwitansi" class="display table table-hover table-bordered table-striped" cellspacing="0" width="100%">
						...
						<tbody>
							...
							foreach($list_pasien as $row){
								$tagihan_ruang_ranap = $this->mperusahaan->get_tagihan_ruangan_ranap($row->no_ipd);
								// var_dump($tagihan_ruang_ranap);die();
						?>
							<tr>
                              <td><?php echo $i++;?></td>
										...
										foreach($tagihan_ruang_ranap as $r){
											if($r['no_ipd'] == $row->no_ipd){
												...
												if($r['tglkeluarrg'] != null){
													...
													$start = new DateTime($r['tglmasukrg']);//start
														$end = new DateTime($row->tgl_keluar);//end
							  ...
							  <td>
									<a href="<?php echo site_url('piutang/cperusahaan/pasien_lunas_ranap/'.$row->no_ipd.'/'.$id_kontraktor.'/'.$date1.'/'.$date2); ?>" class="btn btn-default btn-sm"><i class="fa fa-book"></i>Lunas</a>
							  </td>
							</tr>
						<?php
						...
						<tr>
							...
							<td style="width:40%;">
								<div class="input-group">
									...
									<a href="<?php echo site_url('piutang/cperusahaan/cetak_tagihan_ranap/'.$id_kontraktor.'/'.$date1.'/'.$date2); ?>" class="btn btn-primary btn-sm" target = "_blank">Cetak Tagihan</a>
								</div>
							</td>
							<td style="width:40%;"></td>
						</tr>
