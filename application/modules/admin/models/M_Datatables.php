 ...
 */
    class M_Datatables extends CI_Model
    {
        ...
        function get_tables($tables,$cari = '',$iswhere = '')
        {
            ...
            $sql = $iswhere!=""?$this->db->query("SELECT * FROM ".$query." WHERE ".$iswhere):$this->db->query("SELECT * FROM ".$query);

            $sql_count = $sql->num_rows();

            $cari = $cari !="" ?implode(" LIKE '%".$search."%' OR ", $cari)." LIKE '%".$search."%'":'';
            ...
            $sql_data = $iswhere!=""?$this->db->query("SELECT * FROM ".$query." WHERE $iswhere AND (".$cari.")".$order." LIMIT ".$limit." OFFSET ".$start):$this->db->query("SELECT * FROM $query $order LIMIT $limit OFFSET $start");
            ...
            if(isset($search))
            {
                ...
                // }
                $sql_cari = $iswhere!=""?$this->db->query("SELECT * FROM ".$query." WHERE $iswhere (".$cari.")"):$this->db->query("SELECT * FROM $query");
                $sql_filter_count = $sql_cari->num_rows();
            }else{
                $sql_filter = $iswhere!=""?$this->db->query("SELECT * FROM ".$query."WHERE ".$iswhere):$this->db->query("SELECT * FROM ".$query);
