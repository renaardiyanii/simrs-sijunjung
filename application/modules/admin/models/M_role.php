...
class M_role extends CI_Model{
	...
	function get_menu_all($id){
		return $this->db->query("select b.page_id,
			CASE WHEN b.is_parent = '1' THEN CONCAT(cast(b.page_id as varchar),'00')
				ELSE CONCAT(cast(b.parent_id as varchar),cast(b.position as varchar))
			END AS urutan,
			CASE WHEN (b.is_parent = '1' or b.parent_id = 0) THEN b.title
				ELSE CONCAT('<i class=\"fa fa-angle-double-right fa-fw\"></i>',b.title)
			END AS title, COALESCE(a.role_id,0) as sts
	...
	function get_data_edit_role($id)
	{
		return $this->db->query("SELECT * FROM dyn_role WHERE id=".$id);
	}

	function roleMenuEdit($id,$data)
	{
		$this->db->where('id', $id);
		$this->db->update('dyn_role', $data); 
		return true;
	}
