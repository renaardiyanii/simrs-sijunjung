<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Rdmregistrasi extends CI_Model{
		...
		*/
		...
		function get_kontraktor_kerjasama($tipe){			
			return $this->db->query("SELECT *, id_kontraktor as id 
						FROM kontraktor 
						WHERE bpjs='$tipe'
						ORDER BY nmkontraktor");
		...
		function get_daftar_pasien_belum_pulang(){
			return $this->db->query("SELECT *, (select nm_poli from poliklinik where id_poli='BA00') as nm_poli
			FROM daftar_ulang_irj du, data_pasien A
			where du.no_medrec=A.no_medrec 
			and du.id_poli = 'BA00'
...
//			return $this->db->query("SELECT * FROM data_pasien a LEFT JOIN tni_hubungan b on a.nrp_sbg=b.hub_id where a.alamat LIKE '%$alamat%'");
		...
		function insert_daftar_ulang($data){
			...
			// LPAD(max(right(no_register,6)),6,'0')
			// $this->db->set('no_register', "(SELECT IFNULL(CONCAT('".$awalan."', LPAD (max(right(no_register,6))+1 ,6,0) ),'".$awalan."000001')
						...
						// TO_CHAR(CAST(LPAD(max(right('RJ2021000002',6)),6,'0') AS INTEGER) + 1,'fm000000')


			// $this->db->set('no_register', "(
			// 	SELECT COALESCE(NULLIF(CONCAT('".$awalan."',TO_CHAR(CAST(LPAD(max(right((SELECT COALESCE( NULLIF((SELECT no_register FROM daftar_ulang_irj),'') , '".$awalan."000001' )),6)),6,'0') AS INTEGER) + 1,'fm000000')),''),'".$awalan."000001'))", FALSE);

			// $this->db->set('no_register',"(SELECT CONCAT('".$awalan."',TO_CHAR(CAST(LPAD(max(right(no_register,6)),6,'0') AS INTEGER) + 1,'fm000000')) FROM daftar_ulang_irj)")
				// $this->db->set('no_register',"(SELECT COALESCE( NULLIF(TO_CHAR(CAST(LPAD(max(right(no_register,6)),6,'0') AS INTEGER) + 1,'fm000000'),'') , 'RJ21000001' )FROM daftar_ulang_irj)");

				//;
				$this->db->set('no_register',"(select '$awalan' || right('000000' || cast( cast(COALESCE((SELECT right(max(no_register),6) FROM daftar_ulang_irj), '000001') as int) +1 as varchar),6) as id)", FALSE);
...
order by no_register desc limit 1");
		...
		function getdata_identitas($no_cm){
			return $this->db->query("select dp.*
...
from dokter_poli a where a.id_dokter='$id_dokter'");
		...
		/**
		 * Added @aldi
		 * Pencarian Id Dokter Berdasarkan Id Hfis
		 * 14/10/2022
		 */

		function cari_iddokter_hfis($hfis)
		{
			return $this->db->select('id_dokter,nm_dokter')->where('kode_dpjp_bpjs',$hfis)->get('data_dokter');
		}
