...
$(function() {
	...
	$(document).on("click",".create_sep",function() {
	    ...
	    swal({
	      ...
	      showLoaderOnConfirm: true,
	        ...
	        $.ajax({
	          type: "POST",
	          url: "<?php echo base_url().'bpjs/sep/insert_sep/'; ?>"+no_register,
	          dataType: "JSON",    
	          success: function(data) {  
				if (data.metaData.code === '200') {
					$('#no_sep').val(data.response.sep.noSep);
					swal.close(); // ⬅️ TUTUP swal secara manual
	...
	$(document).on("click",".create_sjp",function() {
	    ...
	    swal({
	      ...
	      showLoaderOnConfirm: true,
	        ...
	        $.ajax({
	          type: "POST",
	          url: "<?php echo base_url().'ird/Rdcsjp/cetak_sjp/'; ?>"+no_register,
	          dataType: "JSON",    
	...
	$('.auto_search_by_nocm').autocomplete({
		serviceUrl: '<?php echo site_url();?>/ird/rdcautocomplete/data_pasien_by_nocm',
		onSelect: function (suggestion) {
			$('#cari_no_cm').val(''+suggestion.no_cm);
			$('#no_medrec_baru').val(''+suggestion.no_cm);
		}
	});

	$('.auto_search_by_nocm_lama').autocomplete({
		serviceUrl: '<?php echo site_url();?>/ird/rdcautocomplete/data_pasien_by_nocm_lama',
		onSelect: function (suggestion) {
	...
	$('.auto_search_by_nokartu').autocomplete({
		serviceUrl: '<?php echo site_url();?>/ird/rdcautocomplete/data_pasien_by_nokartu',
		onSelect: function (suggestion) {
	...
	$('.auto_search_by_noidentitas').autocomplete({
		serviceUrl: '<?php echo site_url();?>/ird/rdcautocomplete/data_pasien_by_noidentitas',
		onSelect: function (suggestion) {
	...
	$('.auto_search_by_nonrp').autocomplete({
		serviceUrl: '<?php echo site_url();?>/ird/rdcautocomplete/data_pasien_by_nonrp3',
		onSelect: function (suggestion) {
	...
	$('.auto_search_by_nama').autocomplete({
		serviceUrl: '<?php echo site_url();?>/ird/rdcautocomplete/data_pasien_by_nama'
		/*onSelect: function (suggestion) {
			$('#cari_no_identitas').val(''+suggestion.no_identitas);
		}
		*/
	...
	$('.auto_search_by_alamat').autocomplete({
		serviceUrl: '<?php echo site_url();?>/ird/rdcautocomplete/data_pasien_by_alamat'
		/*onSelect: function (suggestion) {
			$('#cari_no_identitas').val(''+suggestion.no_identitas);
		}
		*/
...
function cetak_tracer(no_register) {
      var windowUrl = '<?php echo base_url();?>ird/tracer/cetak/'+no_register;
      window.open(windowUrl,'p');
}

function tindakan(no_register) {
      var windowUrl = '<?php echo base_url();?>ird/rdcpelayanan/form/tindakan/'+no_register;
      window.open(windowUrl,'p');
}

function cetak_identitas(no_medrec) {
      var windowUrl = '<?php echo base_url();?>ird/rdcregistrasi/cetak_identitas/'+no_medrec;
      window.open(windowUrl,'p');
}
    ...
    <div class="col-lg-12 col-md-12">
        <div class="card">
			...
			<?php 
					...
					echo form_open('ird/rdcregistrasi/pasien', $attributes);?>
                <div class="row p-t-10 m-b-15">
                    ...
                    <div class="col-md-5">
                        ...
                        <?php $url=site_url('ird/rdcregistrasi/regpasien'); ?>
                    <div class="col-md-4">
                    ...
                    <table id="tbl-pasien" class="display nowrap table table-hover table-bordered table-striped" cellspacing="0">
						...
						<tbody>
							...
							if ($data_pasien!="") {
								...
								<tr>
								   <td>
										<a href="<?php echo site_url('ird/rdcregistrasi/daftarulang/'.$row->no_medrec); ?>" class="btn btn-primary btn-xs" style='width:85px;margin-bottom: 3px;'>Daftar Ulang</a><br>
										...
										<!-- <a href="<?php echo site_url('ird/rdcpelayanan/cetak_resume/').$row->no_medrec; ?>" class="btn btn-danger btn-xs" target='_blank' style='width:85px;'>Cetak PRMRJ</a><br> -->
										<a href="<?php echo site_url('ird/rdcregistrasi/st_cetak_kartu_pasien/').'/'.$row->no_cm; ?>" target="_blank" class="btn btn-danger btn-xs"> Cetak Kartu Pasien</a>
										<!-- <a href="<?php echo site_url('irj/rjcregistrasi/cetak_label_cm/').$row->no_cm; ?>" target="_blank" class="btn btn-info btn-xs"> Cetak Label</a> -->
									</td>
...
<div><h3>History Kunjungan Pasien</h3></div>
                  <div class="table-responsive m-t-0">
