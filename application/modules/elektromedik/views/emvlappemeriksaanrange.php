...
function tablelaprange(date0,date1){
	var url = "<?php echo site_url();?>elektromedik/Emclaporan/showlap_pemeriksaan/";
	if(date0!='' && date1!=''){
		url = url+date0+"/"+date1;	
	}
	table = $('#lapem').DataTable({
        ajax: url,
        columns: [
            { data: "idtindakan" },
            { data: "nmtindakan" },
...
function down_excel(){
    ...
    if(date0=='' && date1==''){
		window.open('<?php echo site_url('elektromedik/Emclaporan/excel_lappemeriksaan');?>/', '_blank'); 
    }else{    	
    	window.open('<?php echo site_url('elektromedik/Emclaporan/excel_lappemeriksaan');?>/'+date0+"/"+date1, '_blank');    
    }
}
...
<section class="content">
	<div class="row">
		<div class="card card-block" style="width:97%;margin:0 auto">
			<div class="card-title">	Laporan Pemeriksaan Elektromedik				
			</div>
			<div class="">				
				<table id="lapem" class="display" cellspacing="0" width="100%">
					<thead>
						<tr>
							<th>No</th>
							<th>Nama Tindakan</th>
