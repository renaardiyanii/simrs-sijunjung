...
class Rimkelas extends CI_Model {
	...
	public function get_all_kelas_with_empty_bed(){
		$data=$this->db->query("
			select a.kelas,a.idrg, count(*), b.nmruang,b.lokasi
			from bed as a
			inner join ruang as b on a.idrg = b.idrg
			where a.isi = 'N'
			and a.aktif is null
			and b.aktif = 'Active'
			group by a.kelas,a.idrg,b.nmruang,b.lokasi
			order by a.idrg");
		return $data->result_array();
	}

	public function get_all_kelas_with_empty_bed_(){
		$data=$this->db->query("
				SELECT
				kelas,
				nmruang,
	...
	public function get_all_status_bed()
	{
		return $this->db->query("SELECT a.kelas,a.idrg, count(*), b.nmruang
		from bed as a
		inner join ruang as b on a.idrg = b.idrg
		where a.isi = 'N' and a.aktif is null
		group by a.kelas,a.idrg,b.nmruang")->result_array();
	}

	public function get_all_empty_bed_by_kelas_and_ruang($kelas,$idrg){
		$query = " select * from bed as a inner join ruang as b on a.idrg = b.idrg where a.isi = 'N'  and a.idrg = '$idrg'";
		// var_dump($query);die();
		$data=$this->db->query($query);
		return $data->result_array();
	}
	
	public function get_all_empty_bed_by_kelas_and_ruang_ibu($kelas,$idrg)
	{
	...
	public function get_all_empty_bed()
	{
		return $this->db->query("select *
		from bed as a
		inner join ruang as b on a.idrg = b.idrg
		where a.isi = 'N'")->result_array();
	}
	...
	public function get_tarif_ruangan($kelas,$idrg){
		...
		// 		from tarif_tindakan
		// 		where kelas = '$kelas'
		// 		AND SUBSTRING (id_tindakan FROM 3 FOR 4) = '$idrg'");
	...
	public function get_bed_perinatologi()
	{
		return $this->db->query("select *,RIGHT(bed, 2) AS no_bed
		from bed 
		where idrg = '1001'")->result();
	}

	public function get_all_bed($kelas,$idrg){
		$query = " select * from bed as a inner join ruang as b on a.idrg = b.idrg where a.idrg = '$idrg'";
		$data=$this->db->query($query);
		return $data->result_array();
	}
