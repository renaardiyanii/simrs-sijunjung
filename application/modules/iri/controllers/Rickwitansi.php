...
class Rickwitansi extends Secure_area {
	...
	public function index(){
		$data['title'] = 'Daftar Kwitansi Rawat Inap Umum';
		$data['reservasi']='';
		$data['daftar']='';
		$data['pasien']='active';
		$data['mutasi']='';
		...
		$data['jenis'] = 'UMUM';
		if ($date!='') { 
			$data['date'] = date('d-m-Y, h:i:s A',strtotime($date));
			$data['list_pasien']=$this->rimpasien->select_pasien_iri_pulang_belum_cetak_kwitansi_by_date($date);
		
		}else{
			$data['date'] = date('Y-m-d');
			$data['list_pasien']=$this->rimpasien->select_pasien_iri_pulang_belum_cetak_kwitansi($data['date']);
		
		}
		$this->load->view('iri/list_pasien_keluar',$data);
	}

	public function kwitansi_ranap_iri(){
		$data['title'] = 'Daftar Kwitansi Rawat Inap BPJS';
		$data['reservasi']='';
		$data['daftar']='';
		$data['pasien']='active';
		$data['mutasi']='';
		...
		if ($date!='') { 
			$data['date'] = date('d-m-Y, h:i:s A',strtotime($date));
			$data['list_pasien']=$this->rimpasien->select_pasien_iri_pulang_belum_cetak_kwitansi_by_date_bpjs($date);
		
		}else{
			$data['date'] = date('Y-m-d');
			$data['list_pasien']=$this->rimpasien->select_pasien_iri_pulang_belum_cetak_kwitansi_bpjs($data['date']);
  ...
  //mmc
	...
	// 	// 	$data['list_ok_pasien'] = "";
	// 	// 	$data['list_pa_pasien'] = "";
	// 	// 	$data['list_lab_pasien'] = "";
	// 	// 	$data['list_radiologi'] = "";//belum ada no_register
	// 	// 	$data['list_resep'] = $this->rimpasien->get_list_resep_pasien($no_ipd,$pasien[0]['noregasal']);
	// 	// 	$data['list_tindakan_ird'] = "";
	// 	// 	$data['poli_irj'] = "";
	// 	// }
	...
	// 	 // print_r($data['grand_total_pembulatan']);die()
	...
	// 	// }
	// 	$data['biaya_administrasi'] = 7/100 * $data['total'];
   ...
   //sriwijaya
	...
	public function detail_kwitansi($no_ipd='',$jenis = ''){
		$data['title']='Kwitansi Rawat Inap Pasien | '.$no_ipd;
		$data['reservasi']='';
		$data['daftar']='';
		$data['pasien']='';
		...
		//}else {
			...
			// $data['list_elektromedik'] = $this->rimpasien->get_list_radiologi_pasien($no_ipd,$pasien[0]['noregasal']);
			// $data['list_resep'] = $this->rimpasien->get_list_resep_pasien($no_ipd,$pasien[0]['noregasal']);
			// $data['list_tindakan_ird'] = "";
			// $data['poli_irj'] = "";
		//}
			$kls = $this->input->post('kls');
			$jatahkls = $this->input->post('jatahkls');
			$get_jatahkls = $pasien[0]['jatahklsiri'];
			$get_kls = $pasien[0]['klsiri'];
			$data['get_tglmasuk'] = $pasien[0]['tglmasukrg'];
			$data['get_tglkeluar'] = $pasien[0]['tgl_keluar'];
			$data['get_kls'] = $get_kls;
			$data['get_jatahkls'] = $get_jatahkls;

			$data['kelas_inacbg'] =$this->rimpasien->get_tarif_kelas_inacbg_new($get_kls)->result();
			// $data['vip_inacbg'] = $this->rimpasien->get_tarif_vip_new()->result();
			$data['jatahkls_inacbg'] =$this->rimpasien->get_tarif_jatahkls_inacbg_new($get_jatahkls)->result();

			$data['cbg1'] = $this->rimpasien->get_tarif_inacbg_kelas_satu()->result();
			$data['cbg2'] = $this->rimpasien->get_tarif_inacbg_kelas_dua()->result();
			
			$data['list_ok_pasien'] =$this->rimpasien->get_list_ok_iri($no_ipd);
			$data['list_pa_pasien'] = "";
			$data['list_lab_pasien'] = $this->rimpasien->get_list_lab_iri($no_ipd);
			$data['list_radiologi'] = $this->rimpasien->get_list_rad_iri($no_ipd);
			//var_dump($this->rimpasien->get_list_rad_iri($no_ipd)); die();
			$data['list_elektromedik'] = $this->rimpasien->get_list_em_iri($no_ipd);
			$data['list_resep'] = $this->rimpasien->get_list_resep_iri_rekap($no_ipd)->result();
			$data['list_resep_iri'] = $this->rimpasien->get_list_resep_iri($no_ipd);
			$data['list_resep_igd'] = $this->rimpasien->get_list_resep_iri_rekap($pasien[0]['noregasal'])->result();

			$tot_obat = 0;
			$obat_igd = 0;

			foreach($data['list_resep'] as $resep){
				$tot_obat += $resep->total_rekap;
			}

			foreach($data['list_resep_igd'] as $resep_igd){
				$obat_igd += $resep_igd->total_rekap;
			}

			if($tot_obat != 0){
				$data['tot_obat'] = $tot_obat;
			}else{
				$data['tot_obat'] = 0;
			}

			if($obat_igd != 0){
				$data['obat_igd'] = $obat_igd;
			}else{
				$data['obat_igd'] = 0;
			}
			//  var_dump($data['tot_obat']);die();
	...
	//	$data['poli_irj'] = $this->rimpasien->get_list_poli_rj_pasien($pasien[0]['noregasal']);
		...
		$ratusan = substr($uang, -3);
		 ...
		 if($data['status_paket'] != 1){
		 	$data['biaya_administrasi'] = 0;//$data['grand_total'] * 5 / 100;
	//	 	buat pembulatan ke atas 500
			$uang = $data['biaya_administrasi'];
			$ratusan = substr($uang, -3);
			 if($ratusan % 1000 != 0){
	...
	public function detail_kwitansi_selesai($no_ipd=''){
		...
		if($pasienold[0]['cetak_kwitansi']=='1' || $pasienold[0]['tgl_cetak_kw'] != ''){
			...
			$data['list_resep'] = $this->rimpasien->get_list_resep_pasien_bpjs($no_ipd);
			$data['list_tindakan_ird'] = $this->rimpasien->get_list_tindakan_ird_pasien($pasien[0]['noregasal']);
			$data['poli_irj'] = $this->rimpasien->get_list_poli_rj_pasien($pasien[0]['noregasal']);
		}else{
			$data['list_lab_pasien'] = $this->rimpasien->get_list_lab_pasien($no_ipd,$pasien[0]['noregasal']);
	...
	public function get_grandtotal_all($no_ipd){
		...
		if ($pasienold[0]['id_poli'] == 'BA00') {
			...
			if(($list_lab_ird)){
			 ...
			 if(($list_resep_ird)){
				$result = $this->string_table_resep_simple($list_resep_ird);
				$grand_total = $grand_total + $result['subtotal'];	
...
//modul cetak laporan simple
	...
	private function string_table_tindakan_simple($list_tindakan_pasien, $pasien){
		...
		// var_dump($list_tindakan_pasien);die();
			...
			foreach ($list_tindakan_pasien as $r) {
				$subtotal_tot = $subtotal_tot+(($r['tumuminap'] + $r['tarif'])*$r['qtyyanri']);
				$subtotal += $r['tarif'];
	...
	private function string_table_ok_simple($list_ok_pasien){
		...
		// 		foreach ($list_ok_pasien as $r) {
		// 			$subtotal = $subtotal + ($r['tarif_jatah']*$r['qty']);
		// 			$biaya_ok = number_format($r['tarif_jatah'],0);
		// 			$vtot = number_format($r['vtot'],0);
		// 		}
		// 		$konten = $konten.'
		// 				<tr>
		...
		// 			if($list_ok_pasien[0]['tgl_masuk'] >= '2023-04-06') {
		// 				$subtotal = $subtotal + ($r['tarif_jatah_bpjs']*$r['qty']);
		// 				$biaya_ok = number_format($r['tarif_jatah_bpjs'],0);
		// 				$vtot = number_format($r['vtot'],0);
		// 			} else if($list_ok_pasien[0]['tgl_masuk'] < '2023-04-06') {
		// 				$subtotal = $subtotal + ($r['tarif_jatah']*$r['qty']);
		// 				$biaya_ok = number_format($r['tarif_jatah'],0);
		...
		// 		foreach ($list_ok_pasien as $r) {
		// 			$subtotal = $subtotal + ($r['tarif_iks']*$r['qty']);
		// 			$biaya_ok = number_format($r['tarif_iks'],0);
		// 			$vtot = number_format($r['vtot'],0);
		// 		}
		// 		$konten = $konten.'
		// 				<tr>
	...
	private function string_table_resep_simple($list_resep, $tuslah=0){
		$konten = "";
	...
	public function cetak_list_pembayaran_pasien($no_ipd=''){
		...
		$konten2 = "
			...
			<hr><br/><br/>";
		$konten2 = $konten2.$this->string_data_pasien($pasien,0,'');
		$grand_total = 0;
		//mutasi ruangan pasien
		if(($list_mutasi_pasien)){
			$result = $this->string_table_mutasi_ruangan($list_mutasi_pasien,$pasien);
			$grand_total = $grand_total + $result['subtotal'];
			$konten2 = $konten2.$result['konten'];
		}
		//tindakan
		if(($list_tindakan_pasien)){
			$result = $this->string_table_tindakan($list_tindakan_pasien);
		...
		';
		$konten2 = $konten.$grand_total_string;
		...
		$konten3 = "
			...
			<hr><br/><br/>";
		$konten3 = $konten3.$this->string_data_pasien($pasien,0,'');
		$grand_total = 0;
		//mutasi ruangan pasien
		if(($list_mutasi_pasien)){
			$result = $this->string_table_mutasi_ruangan($list_mutasi_pasien,$pasien);
			$grand_total = $grand_total + $result['subtotal'];
			$konten3 = $konten3.$result['konten'];
		}
		//tindakan
		if(($list_tindakan_pasien)){
			$result = $this->string_table_tindakan($list_tindakan_pasien);
		...
		';
		$konten3 = $konten.$grand_total_string;
	...
	public function cetak_surat_adm($no_ipd)
    {     
		...
		$data['penanggung_jawab'] = $this->rimpasien->get_data_penanggung_jwb($no_ipd);
		$data['data_pasien'] = $this->rimpasien->get_data_pasien_adm($no_ipd);
		$data['cterbilang']=new rjcterbilang();
		$data['vtot_terbilang']=$data['cterbilang']->terbilang($data['data_pasien']->uang_muka_adm);
		//  var_dump($data['vtot_terbilang']);die();
        $this->load->view('iri/surat_administrasi', $data);
	}
