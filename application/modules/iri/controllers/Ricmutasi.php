...
class Ricmutasi extends Secure_area {
	...
	public function get_empty_bed_select(){
		...
		$kelas = $this->rimkelas->get_all_empty_bed_by_kelas_and_ruang($temp1[2],$temp1[0]);
		// var_dump($kelas);
		$string_option = "";
		foreach ($kelas as $r) {
			$string_option = $string_option.'<option value="'.$r['bed'].'">'.explode(' ',$r['bed'])[2].'</option>';
	...
	public function get_empty_bed_select_mutasi(){
		$temp = $this->input->post('val');
		$temp1 = explode("@", $temp);
		
		$kelas = $this->rimkelas->get_all_bed($temp1[2],$temp1[0]);
		$string_option = "";
		foreach ($kelas as $r) {
			$string_option = $string_option.'<option value="'.$r['bed'].'">'.explode(' ',$r['bed'])[2].'</option>';
		}
	...
	public function get_empty_bed_select_ibu(){
		$temp = $this->input->post('val');
		$temp = explode("-", $temp);
		$kelas = $this->rimkelas->get_all_empty_bed_by_kelas_and_ruang_ibu($temp[2],$temp[0]);
		$string_option = "";
		foreach ($kelas as $r) {
			$string_option = $string_option.'<option value="'.$r['bed'].'">'.explode(' ',$r['bed'])[2].'</option>';
			// $string_option = $string_option.'<option value="'.$r['bed'].'">'.$r['bed'].'</option>';
	...
	public function mutasi($no_ipd)
	{
		$data['title'] = 'Mutasi Ruangan';
		$data['kelas'] = $this->rimkelas->get_all_kelas_with_empty_bed();
		...
		$data['data_pasien']=$this->rimpendaftaran->get_data_pasien($no_ipd)->row();
		$data['pasien']=$this->rimpendaftaran->get_data_pasien_iri($no_ipd);
		$data['histori_ruang']=$this->rimpendaftaran->get_histori_ruang_pasien($no_ipd)->result();


		// $data['bed'] = $this->rimpasien->get_kelas()->result();
		// $data['ruangan'] = $this->rimpasien->get_kelas()->result();
 
		$this->load->view('rivmutasiruangan',$data);

	}

	public function mutasi_ruangan()
	{
		 if($this->input->post('tgl_masuk') != ''){
			// var_dump($this->input->post('tgl_masuk'));die();
			$no_ipd=$this->input->post('no_ipd');
			$temp_ruang_raw=$this->input->post('ruang2');
			$temp_ruang =explode("@", $temp_ruang_raw);

			$data_ruang_iri['kelas']=explode("@", $temp_ruang_raw)[2]; // Kelas			
			$data_ruang_iri['idrg']=$temp_ruang[0];
			$data_ruang_iri['bed']=$this->input->post('bed2');
			$login_data = $this->load->get_var("user_info");
			$data_ruang_iri['xuser']=$login_data->username;
			$no_id = $this->rimpendaftaran->select_ruang_iri_new()->row()->idrgiri;
			$data_ruang_iri['idrgiri']=$no_id + 1;
			//ngambil dari jatah kelas
			$data_ruang_iri['harga_jatah_kelas']= 0;
			$data_ruang_iri['tglmasukrg']=$this->input->post('tgl_masuk');
			$data_ruang_iri['tglkeluarrg']=$this->input->post('tgl_keluar');
			$data_ruang_iri['statmasukrg']="pindahan";
			$data['statkeluarrg'] = 'pindah';

			$get_tarif_ruang = $this->rimkelas->get_tarif_ruangan($data_ruang_iri['kelas'],$data_ruang_iri['idrg'])->row();
			...
			$pasien = $this->rimpendaftaran->get_data_pasien_iri($no_ipd);
			$get_idrgiri = $this->rimtindakan->get_idrgiri_baru($no_ipd)->row();
	
			// update table bed nya none isi nya u/ bed asal
			$data_bed = 'N';
			$idrgasal = $get_idrgiri->idrg;
			$bedasal = $get_idrgiri->bed;
			$kelasasal = $get_idrgiri->kelas;
			$this->rimtindakan->update_bed($data_bed,$idrgasal,$bedasal,$kelasasal);
			
	
			//update bed baru isi nya jadi Y
			$data_bednew = 'Y';
			$idrgnew = explode("@", $temp_ruang_raw)[0];
			$bednew = $this->input->post('bed');;
			$kelasnew = explode("@", $temp_ruang_raw)[2];
			$this->rimtindakan->update_bed($data_bednew,$idrgnew,$bednew,$kelasnew);
	
			// update table ruang iri 
			...
			//ngambil dari jatah kelas
			$data_ruang_iri['harga_jatah_kelas']= 0;
			$data_ruang_iri['tglmasukrg']=date('Y-m-d');
			...
			if($pasien[0]['titip'] == '1') {
				$data_pendaftaran['titip'] = $titip;
			}	
			$this->rimtindakan->update_pasien_iri_mutasi($no_ipd,$data_pendaftaran);
	...
	public function get_empty_bed_select_ganti_ruang(){
		$temp = $this->input->post('val');
		$temp = explode("-", $temp);
		$temp_kelas = substr($temp[2],6);
		$temp_kelas_2 = str_replace(')','',$temp_kelas);
		$kelas = $this->rimkelas->get_all_empty_bed_by_kelas_and_ruang($temp_kelas_2,$temp[0]);
		$string_option = "";
		foreach ($kelas as $r) {
			$string_option = $string_option.'<option value="'.$r['bed'].'">'.explode(' ',$r['bed'])[2].'</option>';
			// $string_option = $string_option.'<option value="'.$r['bed'].'">'.$r['bed'].'</option>';
	...
	public function hapus_detail($id,$noipd){
		$this->rimpendaftaran->delete_histori_ruang_pasien($id);
		redirect('iri/ricmutasi/mutasi/'.$noipd);
	}
