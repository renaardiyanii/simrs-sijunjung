...
if(cara_bayar == "BPJS"){
	...
	ajaxku = buatajax();
	var url="<?php echo site_url('irj/rjcregistrasi/data_kontraktor_KERJASAMA'); ?>";
	url=url+"/"+cara_bayar;
	url=url+"/"+Math.random();
	ajaxku.onreadystatechange=stateChangedKontraktor;
	ajaxku.open("GET",url,true);
...
<div class="ribbon-wrapper card">
			...
			<div class="ribbon-content">
				<div class="row">
					...
					<div class="col-sm-9">
					<!-- <form class="form-horizontal" action="<?php //echo site_url('iri/ricpasien/update_cara_bayar_new'); ?>" method="post"> -->
						...
						<table class="table-sm table-striped" style="font-size:15">
						  <tbody>
							...
							<tr>
								...
								<td>:&nbsp;</td>
								<td>
								<select class="form-control" name="kelas_pasien_update">
									<option value="VIP" <?= $data_pasien[0]['kelas']=='VIP'?'selected':'' ?>>VIP</option>
									<option value="I" <?= $data_pasien[0]['kelas']=='I'?'selected':'' ?>>I</option>
									<option value="II" <?= $data_pasien[0]['kelas']=='II'?'selected':'' ?>>II</option>
									<option value="III" <?= $data_pasien[0]['kelas']=='III'?'selected':'' ?>>III</option>
								</select>
							</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td>&nbsp;</td>
								<td>
								<button type="button" id="btn-update-semua" onclick="simpan_perubahan()" class="btn btn-primary btn-sm" ><i class="fa fa-save"></i> UPDATE</button>
							</tr>
							<!-- <tr>
								<th></th>
								<td>&nbsp;</td>
...
<script>
	function simpan_perubahan(){
		$.ajax({
				type: "POST",
				url: "<?php echo site_url('iri/ricpasien/update_cara_bayar_new'); ?>",
				data: $("#myForm").serialize(),
				beforeSend: function(){
					$("#btn-update-semua").attr('disabled',true);
					$("#btn-update-semua").html('Loading....');
				...
				complete:function(){
					$("#btn-update-semua").attr('disabled',false);
					$("#btn-update-semua").html('UPDATE');
