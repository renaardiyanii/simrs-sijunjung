...
//  $an = isset($laporan_anestesi->formjson)?json_decode($laporan_anestesi->formjson):'';
//  var_dump($an->question34);die();
...
function sendDataToServerLaporanAnestesi(survey) {
        ...
        $.ajax({
            type: "POST",
            url: '<?= base_url('iri/rictindakan/laporan_anestesi/') ?>',
            data: {
	...
	<?php }else{ ?>
        survey_laporan_anestesi.data = {"question1":{"asal_ruangan":"<?= $data_pasien[0]['nmruang'] ?>",
			//"danestesi":"<?php //echo isset($get_data_ok->nm_dokter_anes)?$get_data_ok->nm_dokter_anes:'' ?>",
			//"panestesi":"<?php //echo isset($get_data_ok->perawat_anas)?$get_data_ok->perawat_anas:'' ?>"},
