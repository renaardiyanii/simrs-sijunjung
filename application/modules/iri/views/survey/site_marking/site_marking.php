...
surveyJSONSiteMarking = <?php echo file_get_contents("site_marking.json",FILE_USE_INCLUDE_PATH);?>;

function sendDataToServerSiteMarking(survey) {

        $.ajax({
            type: "POST",
            url: '<?= base_url('iri/rictindakan/site_marking/') ?>',
            data: {
    ...
    var survey_site_marking = new Survey.Model(surveyJSONSiteMarking);
    

	<?php if($site_marking->num_rows()){ ?>
		survey_site_marking.data = <?= $site_marking->row()->formjson; ?>
	<?php }else{ ?>
...
// survey.css = myCss;
$("#SurveySiteMarking").Survey({
    model: survey_site_marking,
    onComplete: sendDataToServerSiteMarking
});
</script>