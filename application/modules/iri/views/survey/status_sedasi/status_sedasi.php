...
function sendDataToServerStatusSedasi(survey) {
        ...
        $.ajax({
            type: "POST",
            url: '<?= base_url('iri/rictindakan/status_sedasi/') ?>',
            data: {
    ...
    <?php if($status_sedasi->num_rows()){ ?>
		survey_status_sedasi.data = <?= $status_sedasi->row()->formjson; ?>
	<?php }else{ ?>
        
        survey_status_sedasi.data = {"diagnosis":[{"Column 1":"<?= $data_pasien[0]['nm_diagmasuk'] ?>"}]};
        <?php } ?>
