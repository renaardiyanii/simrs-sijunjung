...
<form id="formInsertPenanggungJawab">
    ...
    <div class="row">
        <div class="col-sm-6">
            <div class="form-group form_bpjs row">
                ...
                <div class="col-sm-9">
                    <div class="input-group">
                        <input type="text" class="form-control" name="no_bpjs" id="no_bpjs" value="<?php echo trim($data_pasien[0]['no_kartu'], ' '); ?>">
            ...
            <!-- <div class="form-group form_bpjs row">
                ...
                <div class="col-sm-9">
                    ...
                    <select class="form-control js-example-basic-single" style="width:100%" name="nmkontraktorbpjs" id="nmkontraktorbpjs">
                        ...
                        ?>
                            <option value="<?php //echo $r['id_kontraktor']; 
                                            ?>"><?php //echo $r['nmkontraktor']; 
                                                ?></option>
                        <?php
                        //}
                        ?>
            ...
            <div class="form-group form_bpjs row hideitem">
                <label class="col-sm-3 col-form-label">No. Rujukan</label>
                <div class="col-sm-9">
                    <input type="text" class="form-control" name="no_rujukan" value="<?php echo isset($pasien_iri[0]->ipdibu)?(!$pasien_iri[0]->ipdibu ? trim($data_pasien[0]['no_kartu'], ' ') : ''):''; ?>" id="no_rujukan">
                </div>
            </div>
            <div class="form-group form_bpjs row hideitem">
    ...
    <button id="buatsepid" type="button" class="btn btn-info" onclick="buatsep()">Buat SEP</button>
...
<!-- modal spri -->
<div class="modal fade modal_suratkontrol" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
            ...
            <div class="modal-body">
                ...
                <div class="formbuatsurkon">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="no_sep_surat_bikin" value="<?php echo trim($data_pasien[0]['no_kartu'], ' '); ?>">
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        <label class="col-sm-3 control-label col-form-label">Dokter Spesialis </label>
                        <div class="col-sm-8">
                            <div class="form-inline" style="width:100%;">
                                <select id="poli_suratkontrol_bikin" class="form-control select2" style="width: 100%;z-index:9999999;" onchange="ambildoktersuratkontrol(this.value)">
...
<script>
    ...
    function ambilriwayatsep(nokartu) {
        $.ajax({
            url: '<?= base_url('irj/rjcregistrasi/get_bpjs_sep_nokartu_spri/') ?>' + '/' + nokartu,
            beforeSend: function() {
            ...
            success: function(data) {
                ...
                index = 1;
                if (data.length > 0) {
                    $('#dpjp_skdp_sep').html(`<option value="${data[0].dokter_bpjs}" selected>${data[0].nama_dokter_bpjs}</option>`)
                    $('#nosurat_skdp_sep').val(data[0].surat_kontrol);
                    // $('#nosurat_skdp_sep').val(data.response.noSPRI);
                    ...
                    //     <tr>
                    //         <td>${index}</td>
                    //         <td>${e.surat_kontrol}</td>
                    //         <td>${e.poli}</td>
                    //         <td>
                    //         <button class="btn btn-xs btn-primary" onclick="parsingsepsuratkontrol('${e.surat_kontrol}')">Gunakan SPRI</button>
                    //         </td>
                    //     </tr>
                    //     `;
                    //     index++;
    ...
    $(document).ready(function() {
        $('.hideitem').hide();
        <?php
        if(isset($pasien_iri[0]->ipdibu)){
        if (!$pasien_iri[0]->ipdibu) {
        ///r_dump($pasien_iri[0]);die();
        ?>
            ambilriwayatsep('<?php echo trim($data_pasien[0]['no_kartu'], ' '); ?>');
        <?php }} ?>
    ...
    function buatsep() {
        $.ajax({
            ...
            data: {
                ...
                'ppkrujukan': '0311R001',
                'catatan': $('#catatan').val(),
                'diagawal': '<?= $pasien_iri[0]->diagmasuk ?? "" ?>',
                'politujuan': '',
                'tujuankunj': '0',
                'flagprocedure': '',
                'kdpenunjang': '',
