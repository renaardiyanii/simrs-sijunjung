...
function sendDataToServermonitdarah(survey) {
	...
	<?php }else{ ?>
        survey_monitoring_darah.data =  {"question20":"<?= isset($data_pasien[0]['nm_dokter'])?$data_pasien[0]['nm_dokter']:'' ?>",
            "question1": <?= json_encode(isset($data_igd->diagnosa) ? str_replace(['(utama)', '(tambahan)'], ["(utama)", "(tambahan)"], $data_igd->diagnosa) : '') ?>
