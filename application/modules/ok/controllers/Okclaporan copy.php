...
class Okclaporan extends Secure_area {
	...
	{
		$data['title'] = 'Laporan Pendapatan Kamar Operasi ';				
	...
	public function download_keuangan($param1='',$param2=''){
		...
		$objPHPExcel->getProperties()->setCreator($namars)  
		        ...
		        ->setDescription("Laporan Keuangan ".$namars."")  
		        ->setKeywords($namars)  
		        ->setCategory("Laporan Keuangan");  
		...
		// Add some data  
      	...
      	$objPHPExcel->getActiveSheet()->setAutoFilter('A3:P3');
		...
		foreach($data_keuangan as $row){
			...
			if($temp == $row->no_ok){
				$objPHPExcel->getActiveSheet()->SetCellValue('M'.$rowCount, $row->jenis_tindakan);
				$objPHPExcel->getActiveSheet()->SetCellValue('N'.$rowCount, $row->biaya_ok);
				$objPHPExcel->getActiveSheet()->SetCellValue('O'.$rowCount, $row->qty);
				$objPHPExcel->getActiveSheet()->SetCellValue('P'.$rowCount, $row->vtot);
				...
				$objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, $row->nm_dokter);
				$objPHPExcel->getActiveSheet()->SetCellValue('M'.$rowCount, $row->dokter_anastesi);
				$objPHPExcel->getActiveSheet()->SetCellValue('N'.$rowCount, $row->biaya_ok);
				$objPHPExcel->getActiveSheet()->SetCellValue('O'.$rowCount, $row->qty);
				$objPHPExcel->getActiveSheet()->SetCellValue('P'.$rowCount, $row->vtot);
				$total_pendapatan = $total_pendapatan + $row->vtot;
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('O'.$rowCount, "Total Pendapatan : ");
		$objPHPExcel->getActiveSheet()->SetCellValue('P'.$rowCount, $total_pendapatan);
		header('Content-Disposition: attachment;filename="'.$filename.'.xls"');  
				
		// Rename worksheet (worksheet, not filename)  
		$objPHPExcel->getActiveSheet()->setTitle('Laporan OK RSU Sriwijaya');    
