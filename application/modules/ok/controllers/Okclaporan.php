...
class Okclaporan extends Secure_area {
	...
	public function download_keuangan($param1='',$param2='')
	{
		...
		$sheet->SetCellValue('A1', "Laporan Pendapatan Kamar Operasi Periode ".date('d F Y', strtotime($param1))." - ".date('d F Y', strtotime($param2)));
		...
		foreach($data_keuangan as $row){
			...
			if($temp == $row->no_ok){
				...
				$sheet->SetCellValue('L'.$rowCount, $row->biaya_ok);
				$sheet->SetCellValue('M'.$rowCount, $row->qty);
				$sheet->SetCellValue('N'.$rowCount, $row->vtot);
				$total_pendapatan = $total_pendapatan + $row->vtot;
			}else {
				$temp = $row->no_ok;
				$sheet->SetCellValue('B'.$rowCount, $row->no_ok);
				...
				$sheet->SetCellValue('J'.$rowCount, $row->kontraktor);
				$sheet->SetCellValue('K'.$rowCount, $row->jenis_tindakan);
				$sheet->SetCellValue('L'.$rowCount, $row->biaya_ok);
				$sheet->SetCellValue('M'.$rowCount, $row->qty);
				$sheet->SetCellValue('N'.$rowCount, $row->vtot);
