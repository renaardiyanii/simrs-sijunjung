...
<div class="card m-5">
	<div class="card-header">
        <?php if(substr($no_register,0,2) == 'RI') { ?>
            <a target="_blank" href="<?= base_url('emedrec/C_emedrec_iri/cetak_serah_terima_asuhan_pasien_iri/'.$no_register.'/'.$data_pasien2[0]['noregasal']) ?>" class="btn btn-primary">Lihat Serah Terima</a>
        <?php } else { ?>
            <a target="_blank" href="<?= base_url('emedrec/C_emedrec_iri/cetak_serah_terima_asuhan_pasien_iri/'.$no_register.'/'.'') ?>" class="btn btn-primary">Lihat Serah Terima</a>
       <?php } ?>
    </div>
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                ...
                                data-ruang_rawat="<?= $json_value->ruang_rawat ?>" 
                                data-diagnosis_medis="<?= $json_value->diagnosis_medis ?>" 
                                data-waktu="<?= isset($json_value->waktu)?$json_value->waktu:'' ?>" 
                                data-profesi="<?= isset($json_value->profesi)?$json_value->profesi:'' ?>"
                                data-petugas1="<?= isset($json_value->petugas1)?$json_value->petugas1:'' ?>" 
                                data-petugas2="<?= isset($json_value->petugas2)?$json_value->petugas2:'' ?>" 
                                data-situations="<?= isset($json_value->situations)?$json_value->situations:'' ?>" 
                                data-background="<?= isset($json_value->background)?$json_value->background:'' ?>" 
                                data-asessment="<?= isset($json_value->asessment)?$json_value->asessment:'' ?>" 
                                data-recommendation="<?= isset($json_value->recommendation)?$json_value->recommendation:'' ?>" > Terapkan Data </button>
                                <?php
                                if(isset($json_value->petugas1) && isset($json_value->petugas2)){
                                    if(explode('-',$json_value->petugas1)[1] == $user->userid ||explode('-',$json_value->petugas2)[1]  == $user->userid){
                                ?>
                                <button type="button"  class="btn btn-info dataedit " data-id="<?= $value->id ?>" 
                                data-ruang_rawat="<?= $json_value->ruang_rawat ?>" 
                                data-diagnosis_medis="<?= $json_value->diagnosis_medis ?>" 
                                data-waktu="<?= $json_value->waktu ?>" 
                                data-profesi="<?= $json_value->profesi ?>" 
                                data-petugas1="<?= $json_value->petugas1 ?>" 
                                data-petugas2="<?= $json_value->petugas2 ?>" 
                                data-situations="<?= $json_value->situations ?>" 
                                data-background="<?= $json_value->background ?>" 
                                data-asessment="<?= $json_value->asessment ?>" 
                                data-recommendation="<?= $json_value->recommendation ?>"
                                > Edit Data </button>
                                <?php }
                                } ?>
                            </td>
...
surveySerahTerima.setValue("ruang_rawat","<?= $nmruang ?>");
// edit data
...
surveySerahTerima.data = {
   "ruang_rawat":"<?= $nmruang ?>",
   "diagnosis_medis":"<?= '( '.$diagmasuk.' ) '.$nm_diagmasuk ?>"
}
<?php }}else{ ?>
    surveySerahTerima.data = {
   "ruang_rawat":"<?= $nmruang ?>",
