	...
	class Okmlaporan extends CI_Model{
		...
		function get_data_kunj_today(){
			return $this->db->query("SELECT b.no_cm, a.no_medrec, a.no_register, nama, a.tgl_kunjungan as tgl, count(1) as banyak 
				...
				GROUP BY a.no_register, b.no_cm, a.no_medrec,nama, tgl ");
		}
		...
		function get_data_kunj_by_date($tgl){
			return $this->db->query("SELECT b.no_cm, a.no_medrec, a.no_register, nama, count(1) as banyak 
				...
				GROUP BY a.no_register,b.no_cm, a.no_medrec,nama ");
		}
		...
		function get_total_kunj($tgl){
			return $this->db->query("SELECT COUNT(*) FROM pemeriksaan_operasi WHERE to_char(tgl_kunjungan,'YYYY-MM-DD')='$tgl' GROUP BY no_register");
		}
		...
		function get_data_keu_tind($awal, $akhir){
			return $this->db->query("SELECT a.* FROM pendapatan_ok as a
					WHERE to_char(a.tgl_kunjungan , 'YYYY-MM-DD') >= '$awal'
					AND to_char(a.tgl_kunjungan , 'YYYY-MM-DD') <= '$akhir'
					AND a.no_ok IS NOT NULL
					ORDER BY a.tgl_kunjungan, a.no_ok");
		}
