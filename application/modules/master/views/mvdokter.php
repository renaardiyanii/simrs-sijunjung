...
function delete_dokter(id){
    if (confirm('Yakin Mengaktifkan Diagnosa ini?')) {
      ...
      $.ajax({
        type:'POST',
        url:"<?php echo base_url();?>master/mcdokter/delete_dokter/"+id,
        data: {
          id: id
        },  
				contentType: false,  
  ...
  function aktif_dokter(id){
    if (confirm('Yakin Mengaktifkan Diagnosa ini?')) {
      // $('#modalLoading').modal('show');
      $.ajax({
        type:'POST',
        url:"<?php echo base_url();?>master/mcdokter/active_dokter/"+id,
        data: {
          id: id
        },  
				contentType: false,  
    ...
    <div class="col-sm-12">
      <div class="card card-outline-primary">
        ...
        <div class="card-block">
          ...
          <table id="example" class="display table table-hover table-striped table-bordered" cellspacing="0" width="100%">
            ...
            <tbody>
              ...
              <tr>
                ...
                <?php endif; ?>
                <td>
                  <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modal-edit" onclick="edit_dokter('<?php echo $row->id_dokter;?>')" style="margin-bottom: 5px;"><i class="fa fa-edit"></i></button>
                  <!-- <a class="btn btn-danger btn-sm" href="<?php echo base_url('master/mcdokter/delete_dokter/'.$row->id_dokter)?>" style="margin-bottom: 5px;"><i class="fa fa-trash"></i> </a>
                  <a class="btn btn-info btn-sm" href="<?php echo base_url('master/mcdokter/active_dokter/'.$row->id_dokter)?>" style="margin-bottom: 5px;"><i class="fa fa-check"></i> </a>                   -->
                  <button type="button" class="btn btn-danger btn-sm" onclick="delete_dokter('<?php echo $row->id_dokter;?>')"><i class="fa fa-trash"></i></button>
                  <button type="button" class="btn btn-success btn-sm" onclick="aktif_dokter('<?php echo $row->id_dokter;?>')"><i class="fa fa-check"></i></button>
                  <button type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#modal-ttd" onclick="add_ttd('<?php echo $row->id_dokter;?>')" style="margin-bottom: 5px;"><i class="fa fa-file"></i></button>
                  <!-- Tombol dengan gambar -->
                  <button type="button" class="btn btn-light btn-sm" onclick="showmodalbpjs('<?php echo $row->poli_bpjs ?>','<?php echo $row->id_dokter ?>')">
                      <img src="<?php echo base_url('assets/images/bpjs.png'); ?>" alt="BPJS" style="width: 24px; height: 24px;">
                  </button>
                </td>
              </tr>
              <?php }} ?>
          ...
          <form method="POST" id="ttd_form" class="form-horizontal"> 
          <!-- Modal ttd Obat -->
          <div class="modal fade" id="modal-ttd" role="dialog" aria-labelledby="modal-ttd" aria-hidden="true">
            <div class="modal-dialog modal-success modal-lg">

              <!-- Modal content-->
              <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title"><b>Tambah TTD</b></h4>
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                </div>
                <div class="modal-body">
                      <div class="form-group row">                        
                        ...
                        <div class="col-sm-8">
                          <input type="text" class="form-control" name="ttd_id_dokter" id="ttd_id_dokter" disabled="">
                          <input type="hidden" class="form-control" name="ttd_id_dokter_hidden" id="ttd_id_dokter_hidden">
                        </div>
                      </div>
                      <div class="form-group row">                        
                        <p class="col-sm-4 form-control-label">Nama Dokter</p>
                        <div class="col-sm-8">
                          <input type="text" class="form-control" name="ttd_nm_dokter" id="ttd_nm_dokter" disabled="">
                        </div>
                      </div>
                      <div class="form-group row">                        
                      ...
                      <div class="form-group row">                        
                        ...
                        <div class="col-sm-8">
                        <img id="my_image" name="my_image" alt="ttd belum ada" style="" />                      
                        </div>
                      </div>
                ...
                <div class="modal-footer">
                  ...
                  <button type="button" class="btn btn-danger waves-effect " data-dismiss="modal">Close</button>
                  <button class="btn btn-info waves-effect btn-ttd" type="submit" id="btn-edit">Tambah TTD</button>
                </div>
              </div>
            </div>
          </div>
                  ...
                  <div class="row">
                    <div class="col-sm-7">
                      ...
                      <div class="form-group row">                        
                        <p class="col-sm-4 form-control-label">Nama Dokter</p>
                        <div class="col-sm-8">
                          <input type="text" class="form-control" name="edit_nm_dokter" id="edit_nm_dokter" required>
                        </div>
                      </div>
                      <div class="form-group row">                        
                      ...
                      <div class="form-group row">                        
                        ...
                        <div class="col-sm-8">
                          <input type="text" class="form-control" name="edit_ket" id="edit_ket" required>
                        </div>
                      </div>
                      <div class="form-group row">                      
                        <p class="col-sm-4 form-control-label">Scan TTD</p>
...
<!-- Modal BPJS -->
<div class="modal fade" id="modal-bpjs" tabindex="-1" role="dialog" aria-labelledby="modalBpjsLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalBpjsLabel">Informasi BPJS</h5>
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                  <input type="hidden" id="poliklinik-dokter">
                  <input type="hidden" id="poliklinik-id-dokter">
                  
                    <label for="bpjs-date">Pilih Tanggal:</label>
                    <div class="input-group">
                        <input type="text" class="form-control" id="bpjs-date" placeholder="YYYY-MM-DD">
                        <div class="input-group-append">
                            <button class="btn btn-primary" type="button" id="btn-cari" onclick="cariData()">
                ...
                <div class="table-responsive">
                  <table class="table table-bordered">
                      ...
                      <tbody id="result-pencarian">
                          <tr><td colspan="3" class="text-center">Belum ada pencarian</td></tr>
                      </tbody>
                  </table>
              </div>
            </div>
...
function showmodalbpjs(poli,iddokter){
  $("#poliklinik-dokter").val(poli);
  $("#poliklinik-id-dokter").val(iddokter);
  
  $("#modal-bpjs").modal('toggle');
}

function cariData() {
    let tanggal = document.getElementById("bpjs-date").value;
    let spesialis = document.getElementById("poliklinik-dokter").value;
    let iddokter = document.getElementById("poliklinik-id-dokter").value;

    if (!tanggal) {
        alert("Silakan pilih tanggal terlebih dahulu!");
        return;
    }
    // Ubah format tanggal dari DD/MM/YYYY atau MM/DD/YYYY menjadi YYYY-MM-DD
    let parts = tanggal.split("/");
    // let tanggal;

    if (parts.length === 3) {
        // Jika formatnya MM/DD/YYYY (misalnya 02/07/2025)
        if (parseInt(parts[0]) > 12) {
            // Jika bagian pertama lebih dari 12, berarti DD/MM/YYYY (Indonesia)
            tanggal = `${parts[2]}-${parts[1]}-${parts[0]}`;
        } else {
            // Jika bagian pertama <= 12, bisa jadi MM/DD/YYYY (US format)
            tanggal = `${parts[2]}-${parts[0]}-${parts[1]}`;
        }
    } else {
        // Jika tidak ada format yang jelas, gunakan tanggal apa adanya
        tanggal = rawTanggal;
    }
    // http://192.168.1.139:8080/bpjs/referensi/cari_dokter_hfis?tglpelayanan=2024-01-01&spesialis=SAR&pelayanan=02
    $.ajax({
        url: "<?= base_url() ?>/bpjs/referensi/cari_dokter_hfis",
        type: "GET",
        data: { tglpelayanan: tanggal, spesialis: spesialis, pelayanan: "02" },
        dataType: "json",
        success: function(response) {
            if (response.metaData.code === "200") {
                let list = response.response.list;
                let html = "";
                    ...
                    list.forEach(function(dokter) {
                        html += `
                            <tr>
                                <td>${dokter.kode}</td>
                                <td>${dokter.nama}</td>
                                <td><button class="btn btn-success btn-sm" onclick="pilihDokter('${iddokter}', '${dokter.kode}')">Pilih</button></td>
                            </tr>`;
                    });
                } else {
...
function pilihDokter(id_dokter, kode_dokter) {
    $.ajax({
        url: "<?= base_url('master/mcdokter/edit_dokter_hfis') ?>" + `/${id_dokter}/${kode_dokter}`,
        type: "GET",
        success: function(response) {
            Swal.fire("Sukses", "Dokter berhasil dipilih!", "success");
        },
        error: function() {
            Swal.fire("Error", "Gagal memilih dokter", "error");
        }
    });
}
...
function add_ttd(id_dokter) {
    $.ajax({
      ...
      success: function(data){
	      //alert(data[0].id_dokter);
        $('#ttd_id_dokter').val(data[0].id_dokter);
        $('#ttd_id_dokter_hidden').val(data[0].id_dokter);
        $('#ttd_nm_dokter').val(data[0].nm_dokter);
        $("#my_image").attr("src",data[0].ttd);
      },
      error: function(){
        alert("error");
      }
...
$(document).on('click','.btn-ttd',function(e){
        e.preventDefault();        
    // var signature = $('#tandatangan').signature('toDataURL');
    var signature =	signaturePad.toDataURL(); 
    // var image = signature.replace("image/png", "image/octet-stream");
    // window.location.href=image;
    var id_dokter = $('#ttd_id_dokter_hidden').val();
		$.ajax({
			url: "<?php echo base_url(); ?>master/mcdokter/add_ttd",
			data :{
				foto: signature,
        id_dokter : id_dokter
			},
        ...
        if (data = true) {        
              ...
              swal("Error","TTD gagal disimpan.", "error");
            }
			},
          error:function(event, textStatus, errorThrown) {
