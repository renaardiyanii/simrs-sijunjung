...
<script type='text/javascript'>
  $(document).ready(function() {
    ...
    $(".select2").select2();
      ...
      $('#edit_form').on('submit', function(e){  
        ...
        $.ajax({  
          url:"<?php echo base_url(); ?>master/mcobat_poli/edit_obat",                         
          method:"POST",  
          data:new FormData(this),  
          contentType: false,  
          cache: false,  
      ...
      $('#delete_form').on('submit', function(e){  
        ...
        $.ajax({  
          url:"<?php echo base_url(); ?>master/mcobat_poli/delete_obat",                         
          method:"POST",  
          data:new FormData(this),  
          contentType: false,  
          cache: false,  
  ...
  function edit_obat(id_obat_poli) {
    $.ajax({
      type:'POST',
      dataType: 'json',
      url:"<?php echo base_url('master/mcobat_poli/get_data_edit_obat')?>",
      data: {
        id_obat_poli: id_obat_poli
      },
      success: function(data){
        $('#edit_id_obat_poli').val(data[0].id_obat_poli);
        $('#edit_id_obat_poli_hidden').val(data[0].id_obat_poli);
        var $newOption = $("<option selected='selected'></option>").val(data[0].id_obat_obat_poli).text(data[0].nm_obat)
        
        $('#cari_obat_edit').val(data[0].nm_obat);
        // $("#edit_id_obat").append($newOption).trigger('change');
        $("#edit_id_obat").val(data[0].id_obat_obat_poli);
        $('#edit_id_poli').val(data[0].id_poli_obat_poli).change();
      },
      error: function(){
        alert("error");
      }
  ...
  function aktif_obat(id){
    if (confirm('Yakin Mengaktifkan Diagnosa ini?')) {
      $.ajax({
        type:'POST',
        url:"<?php echo base_url();?>master/mcobat_poli/active_obat/"+id,
        data: {
          id: id
        },  
				contentType: false,  
    ...
    <div class="col-sm-12">
      <div class="card card-outline-primary">
        <div class="card-header">
          <h3 class="text-white">DAFTAR OBAT POLI</h3>
        </div>
        <div class="card-block">
          ...
          <form method="POST" action="<?php echo base_url().'master/mcobat_poli/insert_obat'; ?>" class="form-horizontal">
          ...
          <div class="collapse" id="myModal" role="dialog" data-backdrop="static" data-keyboard="false">
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  ...
                  <div class="form-group row">
                    <div class="col-sm-1"></div>
                    <p class="col-sm-3 form-control-label" id="lbl_nama">Poliklinik</p>
                    <div class="col-sm-6">
                      <select id="satuanb" class="form-control select2" style="width: 100%" name="id_poli" required>
                        <option value="" disabled selected="">-Pilih Poliklinik-</option>
                        <?php 
                          foreach($pilih_poli as $row){
          ...
          <div class="collapse" id="editModal" role="dialog" data-backdrop="static" data-keyboard="false">
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  ...
                  <div class="form-group row">
                    <div class="col-sm-1"></div>
                      <p class="col-sm-3 form-control-label">Id Obat Poli</p>
                      <div class="col-sm-6">
                      <input type="text" class="form-control" name="edit_id_obat_poli" id="edit_id_obat_poli" disabled="">
                      <input type="hidden" class="form-control" name="edit_id_obat_poli_hidden" id="edit_id_obat_poli_hidden">
                    </div>
                  </div>
                  ...
                  <div class="form-group row">
                    <div class="col-sm-1"></div>
                      <p class="col-sm-3 form-control-label">Poliklinik</p>
                      <div class="col-sm-6">
                        <select id="edit_id_poli" class="form-control select2" style="width: 100%" name="edit_id_poli" required>
                          <option value="" disabled selected="">-Pilih Poliklinik-</option>
                          <?php 
                            foreach($pilih_poli as $row){
                              echo '<option value="'.$row->id_poli.'">'.$row->nm_poli.'</option>';
            ...
            <tbody id="bodyt">
              <?php $i=1; foreach ($obat as $row) { ?>
                <tr>
                  ...
                  <td><?php echo $row->id_poli_obat_poli; ?></td>
                  <td>
                    <?php if($row->delete_obat_poli == '0'){
                       echo 'AKTIF' ;
                    }else{
                      echo 'TIDAK AKTIF';
                    } ?>
                  ...
                  <td>
                    <button type="button" title="Edit" class="btn btn-primary btn-sm" data-toggle="collapse" data-target="#editModal" onclick="edit_obat('<?php echo $row->id_obat_poli;?>')"><i class="fa fa-edit"></i></button>
                    <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#deleteModal" onclick="delete_obat('<?php echo $row->id_obat_poli;?>')"><i class="fa fa-trash"></i></button>
                    <button type="button" class="btn btn-success btn-sm" onclick="aktif_obat('<?php echo $row->id_obat_poli?>')" ><i class="fa fa-check"></i></button>
                  </td>
                </tr>
              <?php } ?>  
            </tbody>
