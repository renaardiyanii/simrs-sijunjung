<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Mmruang extends CI_Model{
		...
		FROM
			...
			JOIN (
				SELECT 
					RIGHT(idtindakan, 4) AS idrg_match, 
					tarif 
				FROM 
					jenis_tindakan_new
				WHERE 
					idtindakan LIKE '1A%'
			) jt ON jt.idrg_match = ruang.idrg 
		...
		function get_data_ruang($id_ruang){
			return $this->db->query("SELECT * FROM ruang WHERE idrg='$id_ruang'");
		}
 ...
 FROM ruang, bed WHERE ruang.idrg=bed.idrg AND bed.bed='$bed'");
		...
		function get_all_bed(){
			return $this->db->query("SELECT ruang.idrg, ruang.nmruang, bed.kelas, RIGHT(bed, 2) AS no_bed, bed.isi, bed.bed, bed.status,bed.aktif FROM ruang, bed WHERE ruang.idrg=bed.idrg ORDER BY bed.bed");
		}
		...
		{
			return $this->db->where('bed',$bed)->update('bed',['aktif'=>$isi==''?NULL:$isi]);
		}

		function soft_delete_ruang($id_ruang,$data){
			$this->db->where('id_ruang',$id_ruang);
			$this->db->update('ruang',$data);
			return true;
		}

		function active_ruang($id_ruang,$data){
			$this->db->where('id_ruang',$id_ruang);
			$this->db->update('ruang',$data);
			return true;
		}

		function get_all_ruang_aktif()
		{
			return $this->db->where('aktif','Active')->get('ruang');
		}

		function get_all_bed_aktif()
		{
			return $this->db->select('bed.bed,bed.kelas,bed.kelasjkn,ruang.nmruang')->from('bed')->join('ruang','ruang.idrg = bed.idrg')->where('bed.aktif',null)->get();
		}
	}
?>