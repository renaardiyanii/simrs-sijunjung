...
class Mctindakan_new extends Secure_area {
	...
	public function index(){
		$data['title'] = 'Master Tindakan ICU & HCU'; 
		$data['tindakan']=$this->mmtindakan->get_all_tindakan_dengan_tarif()->result();
		$data['poliklinik']=$this->mmtindakan->get_poliklinik()->result();
		$this->load->view('master/mvtindakan_new',$data);
	}

	public function master_tindakan(){
		$data['title'] = 'Master Tindakan'; 
		$data['tindakan']=$this->mmtindakan->get_all_tindakan_tanpa_tarif()->result();
		$data['poliklinik']=$this->mmtindakan->get_poliklinik()->result();
		$this->load->view('master/mvtindakan_new1',$data);
	}
	...
	public function insert_master_tindakan(){
		...
		$jt['tarif']=$this->input->post('tarif');
		$jt['tmno']=$this->input->post('tmno');
		$jt['instalasi']=$this->input->post('instalasi');
		$jt['idtindakan']=$this->input->post('idtindakan');
		$jt['id_poli']=$this->input->post('id_poli');
		
		
		$this->mmtindakan->insert_jenis_tindakan_tanpa_tarif($jt);
		...
		redirect('master/mctindakan_new','refresh');
	}
	...
	public function get_data_edit_tindakan(){
		...
		$datajson=$this->mmtindakan->get_data_tindakan_new($idtindakan)->result();
	...
	public function soft_delete_tindakan(){
		...
		$this->mmtindakan->delete_tindakan_new($id);
		redirect('master/Mctindakan_new/master_tindakan');
	}

	public function soft_delete_tindakan_dgn_tarif(){
		$id = $this->input->post('delete_id');
		$this->mmtindakan->delete_tindakan_new($id);
		redirect('master/Mctindakan_new');
	}

	public function active_tindakan($id){

		// $this->mmtindakan->delete_tindakan($id);
		$this->mmtindakan->active_tindakan_new($id);
		
		redirect('master/Mctindakan_new/master_tindakan');
		//print_r($data);
	}

	public function active_tindakan_dgn_tarif($id){
		$this->mmtindakan->active_tindakan_new($id);
		redirect('master/Mctindakan_new');
	}
	...
	public function nonactive_tindakan_dgn_tarif($id) {
		$this->mmtindakan->nonactive_tindakan($id);
		redirect('master/Mctindakan_new');
	}
	...
	public function edit_tindakan_new(){
		$idtindakan = $this->input->post('edit_idtindakan');
		$data['nmtindakan'] = $this->input->post('edit_nmtindakan');
		$data['tarif'] = $this->input->post('edit_tarif_tind');
		$data['id_poli'] = $this->input->post('edit_id_poli_tind');

		$this->mmtindakan->edit_jenis_tindakan_new($idtindakan, $data);
		echo json_encode(array("status" => TRUE));
	}

	public function insert_tindakan_dgn_tarif(){
		...
		$id=$this->mmtindakan->insert_jenis_tindakan_tanpa_tarif($jt);		

	

		//insert to tarif_tindakan
		
		if($this->input->post('tarif_hcu')!=0){
			$data['id_tindakan']=$ids;
			$data['kelas'] = "HCU";
			$data['total_tarif']=$this->input->post('tarif_hcu');

			$this->mmtindakan->insert_tarif_tindakan_new($data);

		}

		if($this->input->post('tarif_icu')!=0){
			$data['id_tindakan']=$ids;
			$data['kelas'] = "ICU";
			$data['total_tarif']=$this->input->post('tarif_icu');

			$this->mmtindakan->insert_tarif_tindakan_new($data);

		}

		if($this->input->post('tarif_iccu')!=0){
			$data['id_tindakan']=$ids;
			$data['kelas'] = "ICCU";
			$data['total_tarif']=$this->input->post('tarif_iccu');

			$this->mmtindakan->insert_tarif_tindakan_new($data);

		}

		if($this->input->post('tarif_nicu')!=0){
			$data['id_tindakan']=$ids;
			$data['kelas'] = "NICU/PICU";
			$data['total_tarif']=$this->input->post('tarif_nicu');

			$this->mmtindakan->insert_tarif_tindakan_new($data);

		}

		if($this->input->post('tarif_cito')!=0){
			$data['id_tindakan']=$ids;
			$data['kategory'] = "cito";
			$data['total_tarif']=$this->input->post('tarif_cito');

			$this->mmtindakan->insert_tarif_tindakan_new($data);

		}

		if($this->input->post('tarif_elektif')!=0){
			$data['id_tindakan']=$ids;
			$data['kategory'] = "elektif";
			$data['total_tarif']=$this->input->post('tarif_elektif');

			$this->mmtindakan->insert_tarif_tindakan_new($data);

		}

		if($this->input->post('tarif_ods')!=0){
			$data['id_tindakan']=$ids;
			$data['kategory'] = "ods";
			$data['total_tarif']=$this->input->post('tarif_ods');

			$this->mmtindakan->insert_tarif_tindakan_new($data);

		}

		if($this->input->post('tarif_local')!=0){
			$data['id_tindakan']=$ids;
			$data['kategory'] = "local";
			$data['total_tarif']=$this->input->post('tarif_local');
	...
	public function get_data_edit_tindakan_dgn_tarif(){
		$idtindakan=$this->input->post('idtindakan');
		$datajson=$this->mmtindakan->get_data_tindakan_dgn_tarif($idtindakan)->result();
	    echo json_encode($datajson);
	}

	public function edit_tindakan_dgn_tarif(){
		//edit to jenis_tindakan
		$idtindakan=$this->input->post('edit_idtindakan_hide');
		$jt['idtindakan']=$this->input->post('edit_idtindakan');
		$jt['nmtindakan']=$this->input->post('edit_nmtindakan');
		...
		$this->mmtindakan->edit_jenis_tindakan($idtindakan, $jt);
		  ...
		  //Kelas NK
		if($this->input->post('edit_tarif_hcu')){
			$data['kelas']="HCU";
			$total_tarif = $this->input->post('edit_tarif_hcu');
			if($total_tarif == null || $total_tarif ==''){
				$data['total_tarif'] = 0;
			}else{
				$data['total_tarif'] = $total_tarif;
			}
			$id_tarif_tindakan=$this->mmtindakan->return_tarif($idtindakan, 'HCU')->row()->id_tarif_tindakan;
			$this->mmtindakan->edit_tarif_tindakan_new($id_tarif_tindakan, $data);
		}
		
		if($this->input->post('edit_tarif_icu')){
			$data['kelas']="ICU";
			$total_tarif = $this->input->post('edit_tarif_icu');
			if($total_tarif == null || $total_tarif ==''){
				$data['total_tarif'] = 0;
			}else{
				$data['total_tarif'] = $total_tarif;
			}
			$id_tarif_tindakan=$this->mmtindakan->return_tarif($idtindakan, 'ICU')->row()->id_tarif_tindakan;
			$this->mmtindakan->edit_tarif_tindakan_new($id_tarif_tindakan, $data);
		}

		if($this->input->post('edit_tarif_iccu')){
			$data['kelas']="ICCU";
			$total_tarif = $this->input->post('edit_tarif_iccu');
			if($total_tarif == null || $total_tarif ==''){
				$data['total_tarif'] = 0;
			}else{
				$data['total_tarif'] = $total_tarif;
			}
			$id_tarif_tindakan=$this->mmtindakan->return_tarif($idtindakan, 'ICCU')->row()->id_tarif_tindakan;
			$this->mmtindakan->edit_tarif_tindakan_new($id_tarif_tindakan, $data);
		}

		if($this->input->post('edit_tarif_nicu')){
			$data['kelas']="NICU/PICU";
			$total_tarif = $this->input->post('edit_tarif_nicu');
			if($total_tarif == null || $total_tarif ==''){
				$data['total_tarif'] = 0;
			}else{
				$data['total_tarif'] = $total_tarif;
			}
			$id_tarif_tindakan=$this->mmtindakan->return_tarif($idtindakan, 'NICU/PICU')->row()->id_tarif_tindakan;
			$this->mmtindakan->edit_tarif_tindakan_new($id_tarif_tindakan, $data);
		}

		// if($this->input->post('edit_tarif_elektif')){
		// 	$data['kategory']="elektif";
		// 	$total_tarif = $this->input->post('edit_tarif_elektif');
		// 	if($total_tarif == null || $total_tarif ==''){
		// 		$data['total_tarif'] = 0;
		// 	}else{
		// 		$data['total_tarif'] = $total_tarif;
		...
		// if($this->input->post('edit_tarif_cito')){
		// 	$data['kelas']="NICU/PICU";
		// 	$total_tarif = $this->input->post('edit_tarif_cito');
		// 	if($total_tarif == null || $total_tarif ==''){
		// 		$data['total_tarif'] = 0;
		// 	}else{
		// 		$data['total_tarif'] = $total_tarif;
		...
		// if($this->input->post('edit_tarif_ods')){
		// 	$data['kelas']="NICU/PICU";
		// 	$total_tarif = $this->input->post('edit_tarif_ods');
		// 	if($total_tarif == null || $total_tarif ==''){
		// 		$data['total_tarif'] = 0;
		// 	}else{
		// 		$data['total_tarif'] = $total_tarif;
		...
		// if($this->input->post('edit_tarif_local')){
		// 	$data['kelas']="NICU/PICU";
		// 	$total_tarif = $this->input->post('edit_tarif_local');
		// 	if($total_tarif == null || $total_tarif ==''){
		// 		$data['total_tarif'] = 0;
		// 	}else{
		// 		$data['total_tarif'] = $total_tarif;
