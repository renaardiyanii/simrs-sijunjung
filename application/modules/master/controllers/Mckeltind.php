...
class Mckeltind extends Secure_area {
	...
	public function index(){
		...
		$data['keltind']=$this->mmkeltind->get_all_keltind_new()->result();
	...
	public function insert_keltind(){
		...
		$data['kel_tindakan']=$this->input->post('nama_kel');
		// $data['desc_kel']=$this->input->post('desc_kel');

		// $this->mmkeltind->insert_keltind($data);		
		$this->mmkeltind->insert_keltind_new($data);	
	...
	public function get_data_edit_keltind_new() {
		$id = $this->input->post('idkel_tind');
		$datajson = $this->mmkeltind->get_data_keltind_new($id)->row();
		echo json_encode($datajson);
	}
	...
	public function edit_keltind_new() {
		$id = $this->input->post('edit_idkel_tind_hidden');

		$data['kel_tindakan'] = $this->input->post('edit_nama_kel');
		$this->mmkeltind->edit_keltind_new($id, $data);
	}

	public function delete_keltind($idkel_tind){		
		if($idkel_tind!=''){
			// $datajson = $this->mmkeltind->delete_keltind($idkel_tind);
			$datajson = $this->mmkeltind->delete_keltind_new($idkel_tind);
			$success = 	'<div class="content-header">
						<div class="box box-default">
							<div class="alert alert-success alert-dismissable">
								<button class="close" aria-hidden="true" data-dismiss="alert" type="button">×</button>
