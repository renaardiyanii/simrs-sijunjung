...
class Mcform extends Secure_area {
	...
	public function index(){
		$data['title'] = 'Master Form';

		$data['form']=$this->mmform->get()->result();
		$this->load->view('master/mvform',$data);
	}
	...
	public function insert_role_form(){
		...
		$berhasil = 0;
		$gagal = 0;
		$data_sebelumnya = $this->mmform->get_role_form_by_id_form($data['id_form'])->result();
		...
		foreach($data['role'] as $val){
			...
			if(!in_array($val,$penampung_role_sebelumnya)){
				...
				$req = $this->mmform->insert_role_form($datas);
				if($req){
					$berhasil++;
				}else{
					$gagal++;
		...
		foreach($data_sebelumnya as $val){
			...
			if(!in_array($val->role,$penampung_role_update)){
				$this->mmform->delete_role_form($val->id);
			}
		}
