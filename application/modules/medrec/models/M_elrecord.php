...
class M_elrecord extends CI_Model {
	...
	function get_kunj_ri($no_medrec){
		return $this->db->query("SELECT a.no_ipd , a.tgl_masuk , a.idrg , b.nmruang , a.dokter , c.nm_diagnosa FROM pasien_iri AS a LEFT JOIN ruang as b ON a.idrg = b.idrg LEFT JOIN icd1 as c ON a.diagnosa1 = c.id_icd WHERE no_medrec = '$no_medrec'");
	}
	...
	function get_kunj_farmasi($no_medrec){
		...
		CASE
				...
				( SELECT nm_poli FROM poliklinik WHERE id_poli = A.idrg ) 
				WHEN A.bed = 'Rawat Darurat' THEN
				( SELECT nm_poli FROM poliklinik WHERE id_poli = A.idrg ) 
				ELSE ( SELECT nmruang FROM ruang WHERE idrg = A.idrg ) 
			END,
			b.no_cm 
		FROM
			resep_pasien A,
	...
	function get_data_ok($no_medrec){
		...
		(SELECT nm_dokter from data_dokter where id_dokter=a.id_dokter) as nm_dokter,
		a.cara_bayar,
		CASE WHEN a.bed='Rawat Jalan' THEN (select nm_poli from poliklinik where id_poli=a.idrg)
		WHEN a.bed='Rawat Darurat' THEN (select nm_poli from poliklinik where id_poli=a.idrg)
		ELSE (select nmruang from ruang where idrg=a.idrg)
	   END,
		b.no_cm
	FROM
