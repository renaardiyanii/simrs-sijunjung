<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Medmlaporan extends CI_Model{
        ...
        function get_data_diag_rj_max($tglawal,$tglakhir,$poli){
			if ($tglawal == '-' && $tglakhir == '-') {
				return $this->db->query("SELECT id_diagnosa,diagnosa,sum(L) as L,sum(P) as P, sum(L) + sum(P) as tot 
					from diag_terbesar 
					group by id_diagnosa,diagnosa
					order by  sum(L) + sum(P) desc
					limit 10");	
			}else {
				if ($poli == 'all') {
					return $this->db->query("SELECT id_diagnosa,diagnosa,sum(L) as L,sum(P) as P, sum(L) + sum(P) as tot 
					from diag_terbesar 
					where to_char(tgl_kunjungan,'YYYY-MM-DD') >= '$tglawal' 
					AND to_char(tgl_kunjungan,'YYYY-MM-DD') <= '$tglakhir'
					group by id_diagnosa,diagnosa
					order by  sum(L) + sum(P) desc
					limit 10");		
				}else {
