...
class Frmcreturjual extends Secure_area{
    ...
    function get_list_pasien(){
        ...
        $hasil = $this->Frmmdaftar->get_pengambilan_resep_pasien_selesai()->result();
        ...
        foreach ($hasil as $key =>$value) {
            ...
            if($value->cara_bayar=='UMUM'){
                ...
                if($value->cetak_kwitansi=='1'){
                    $statuscb = ' <br> <b>(Lunas)</b>';
                }else {
                    $statuscb = ' <br> (<i>Belum Lunas</i>)';
                }
            }else {
                $carabayar = $value->cara_bayar;
            }
            ...
            $row2['aksi'] = 
            "<a href='".site_url('farmasi/Frmcreturjual/retur/'.$value->no_register)."' class=\"btn btn-primary btn-sm\">Detail</a>";

            // "<div align='center'>
            // <button class=\"btn btn-primary btn-sm\" data-toggle=\"modal\" data-target=\"#detailModal\" data-id=\"$value->no_resep\">Detail</button> 
            // </div>";
