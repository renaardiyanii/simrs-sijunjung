<?php
$data = (isset($persetujuan_anestesi_sedasi->formjson)?json_decode($persetujuan_anestesi_sedasi->formjson):'');
$result = array_chunk($persetujuan_anestesi_sedasi, 1);
// var_dump($data);
//<?= isset()? $data->info_penelitian->diagnosis->{'Column 2'} == "item1" ? "√":'':'' ?>
   ...
   <link rel="stylesheet" href="<?= base_url('assets/css/paper.css') ?>">
    ...
    if($result){
        ...
        <div class="A4 sheet  padding-fix-10mm">
                ...
                <center><h4>PERSETUJUAN ANESTESI / SEDASI</h4></center>
                ...
                <table id="data" width="100%" border=1 cellpadding="2px">
                    ...
                    <tr>
                        <td colspan="4">Pemberi Informasi : <?= isset($value->question7)? Explode('-',$value->question7)[0]:'' ?></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td><?= isset($value->info_penelitian->indikasi->{'Column 1'})?$value->info_penelitian->indikasi->{'Column 1'}:'' ?></td>
                        <td style="text-align:center"><?php echo isset($value->info_penelitian->indikasi->{'Column 2'}[0])? $value->info_penelitian->indikasi->{'Column 2'}[0] == "item1" ? "√ ":'':'' ?></td>
                    </tr>
                    <tr>
                        <td>5</td>
                        <td>Tata Cara</td>
                    ...
                    <tr>
                        <td>11</td>
                        <td>Manfaat</td>
                        <td><?= isset($value->info_penelitian->manfaat->{'Column 1'})?$value->info_penelitian->manfaat->{'Column 1'}:'' ?></td>
                        <td style="text-align:center"><?php echo isset($value->info_penelitian->manfaat->{'Column 2'}[0])? $value->info_penelitian->manfaat->{'Column 2'}[0] == "item1" ? "√ ":'':'' ?></td>
                    </tr>
                    ...
                    <tr>
                        <td colspan="4"  style="text-align:center;font-size:12px;font-weight:bold">PERSETUJUAN ANESTESI/SEDASI</td>
                    </tr>

                    <tr>
                        <td colspan="4">
                        Yang bertanvaluengan di bawah ini, saya , nama <?= isset($value->mt_bio_wali_pasien->nm)?$value->mt_bio_wali_pasien->nm:'...' ?>, 
                        umur <?= isset($value->mt_bio_wali_pasien->umur)?$value->mt_bio_wali_pasien->umur:'...' ?>.tahun, 
                        <?= isset($value->mt_bio_wali_pasien->jk)?$value->mt_bio_wali_pasien->jk:'...' ?>, alamat <?= isset($value->mt_bio_wali_pasien->alamat)?$value->mt_bio_wali_pasien->alamat:'...' ?>, dengan ini menyatakan persetujuan untuk 

                        dilakukannya tindakan <?= isset($value->question6)?$value->question6:'...' ?> terhadap saya / 

                        <?= isset($value->txt_tindakan)?$value->txt_tindakan:'...' ?> saya* bernama <?= isset($value->mt_bio_pasien1->nama)?$value->mt_bio_pasien1->nama:'...' ?>, 
                        umur <?= isset($value->mt_bio_wali_pasien->umur)?$value->mt_bio_wali_pasien->umur:'...' ?>.tahun, <?= isset($value->mt_bio_wali_pasien->jk)?$value->mt_bio_wali_pasien->jk:'...' ?>, 

                        Alamat <?= isset($value->mt_bio_pasien1->alamat)?$value->mt_bio_pasien1->alamat:'...' ?> Saya memahami perlunya dan manfaat tindakan tersebut 
                        ...
                        kepada izin Tuhan Yang Maha Esa. <br><br><br>
                        <p style="margin-left:450px">Bukitiinggi, <?= isset($value->tgl_setuju_anastesi)? date('d-m-Y h:i',strtotime($value->tgl_setuju_anastesi)):''; ?></p>
        ...
        <?php }}else{ ?>
            <div class="A4 sheet  padding-fix-10mm">
                ...
                <table id="data" width="100%" border=1 cellpadding="2px">
                    ...
                    <tr>
                        <td colspan="4">Pemberi Informasi : <?= isset($data->question7)? Explode('-',$data->question7)[0]:'' ?></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td>4</td>
                        <td>Indikasi Tindakan</td>
                        <td><?= isset($data->info_penelitian->indikasi->{'Column 1'})?$data->info_penelitian->indikasi->{'Column 1'}:'' ?></td>
                        <td style="text-align:center"><?php echo isset($data->info_penelitian->indikasi->{'Column 2'}[0])? $data->info_penelitian->indikasi->{'Column 2'}[0] == "item1" ? "√ ":'':'' ?></td>
                    </tr>
                    <tr>
                        <td>5</td>
                        <td>Tata Cara</td>
                    ...
                    <tr>
                        <td>11</td>
                        <td>Manfaat</td>
                        <td><?= isset($data->info_penelitian->manfaat->{'Column 1'})?$data->info_penelitian->manfaat->{'Column 1'}:'' ?></td>
                        <td style="text-align:center"><?php echo isset($data->info_penelitian->manfaat->{'Column 2'}[0])? $data->info_penelitian->manfaat->{'Column 2'}[0] == "item1" ? "√ ":'':'' ?></td>
                    </tr>
                    ...
                    <tr>
                        <td colspan="4">
                        Yang bertandatangan di bawah ini, saya , nama <?= isset($data->mt_bio_wali_pasien->nm)?$data->mt_bio_wali_pasien->nm:'...' ?>, 
                        umur <?= isset($data->mt_bio_wali_pasien->umur)?$data->mt_bio_wali_pasien->umur:'...' ?>.tahun, 
                        <?= isset($data->mt_bio_wali_pasien->jk)?$data->mt_bio_wali_pasien->jk:'...' ?>, alamat <?= isset($data->mt_bio_wali_pasien->alamat)?$data->mt_bio_wali_pasien->alamat:'...' ?>, dengan ini menyatakan persetujuan untuk 

                        dilakukannya tindakan <?= isset($data->question6)?$data->question6:'...' ?> terhadap saya / 

                        <?= isset($data->txt_tindakan)?$data->txt_tindakan:'...' ?> saya* bernama <?= isset($data->mt_bio_pasien1->nama)?$data->mt_bio_pasien1->nama:'...' ?>, 
                        umur <?= isset($data->mt_bio_wali_pasien->umur)?$data->mt_bio_wali_pasien->umur:'...' ?>.tahun, <?= isset($data->mt_bio_wali_pasien->jk)?$data->mt_bio_wali_pasien->jk:'...' ?>, 

                        Alamat <?= isset($data->mt_bio_pasien1->alamat)?$data->mt_bio_pasien1->alamat:'...' ?> Saya memahami perlunya dan manfaat tindakan tersebut 
                        sebagaimana telah dijelaskan seperti di atas kepada saya, termasuk risiko dan komplikasi 
                        yang mungkin timbul. Saya juga menyadari bahwa oleh karena ilmu kedokteran bukanlah ilmu pasti, 
                        maka keberhasilan tindakan kedokteran bukanlah keniscayaan, melainkan sangat bergantung 
                        kepada izin Tuhan Yang Maha Esa. <br><br><br>
                        <p style="margin-left:450px">Bukitiinggi, <?= isset($data->tgl_setuju_anastesi)? date('d-m-Y h:i',strtotime($data->tgl_setuju_anastesi)):''; ?></p>
                       <table width="100%" border="0">
                           <tr>
                               <td  width="30%">
                                   <center><span style="text-align:center">Yang menyatakan</span></center>
