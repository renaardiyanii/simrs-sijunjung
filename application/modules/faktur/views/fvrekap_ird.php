<?php 
if ($role_id == 1) {
    $this->load->view("layout/header_left");
} else {
    $this->load->view("layout/header_horizontal");
}?>

<script type='text/javascript'>
var intervalSetting = function () { 
location.reload(); 
}; 
setInterval(intervalSetting, 120000);

	$(function() {
		$('#date_picker').datepicker({
			format: "yyyy-mm-dd",
			endDate: '0',
			autoclose: true,
			todayHighlight: true,
		});  
	});
	
	$(document).ready(function() {
		$('#tabel_kwitansi').DataTable();
	} );

</script>
	
<?php
	echo $this->session->flashdata('message_cetak'); 
?>
<section class="content">
	<div class="row">				
		<div class="box" style="width:97%;margin:0 auto">
			<div class="box-header">
				<h3 class="box-title">Daftar Rekap Faktur & Kwitansi Tanggal <b><?php echo $date;?></b></h3>			
			</div>
			<div class="box-body">
				<div class="form-group row">
					<div class="col-md-3">
						<?php echo form_open('faktur/fcrekap/rawat_darurat');?>
							<div class="input-group">
								<input type="text" id="date_picker" class="form-control" placeholder="Tanggal Kunjungan" name="date">
								<span class="input-group-btn">
									<button class="btn btn-primary" type="submit">Cari</button>
								</span>
							</div><!-- /input-group -->
						<?php echo form_close();?>
					</div><!-- /.col-lg-6 -->
						
				</div><!-- /inline -->
				
				<hr>
					<table id="tabel_kwitansi" class="display" cellspacing="0" width="100%">
						<thead>
							<tr>
								<th>No</th>
								<th>Tanggal Kwitansi</th>
								<th>No Kwitansi</th>
								<th>Ket</th>
								<th>No Registrasi</th>
								<th>No Medrec</th>
								<th>Nama</th>
								<th>Cara Bayar</th>
								<th>Poli</th>
								<th>Aksi</th>
							</tr>
						</thead>
						<tbody>
						
							<?php
							// print_r($pasien_daftar);
							$i=1;
							foreach($pasien_daftar as $row){
								$no_register=$row->no_register;
								$no_kwitansi=$row->no_kwitansi;
						?>
							<tr>
							  <td><?php echo $i++;?></td>
							  <td><?php echo date("d-m-Y", strtotime($row->tgl_kunjungan)).' | '.date("H:m", strtotime($row->tgl_kunjungan)); ?></td>
							  <td><?php echo $row->no_kwitansi;?></td>
							  <td><?php 
								if($row->referensi == 1) {
									echo 'Kwitansi Tindakan Administrasi';
								} else {
									echo 'Kwitansi Setelah Poli';
								}
							  ?></td>
							  <td><?php echo $row->no_register;?></td>
							  <td><?php echo $row->no_cm;?></td>
							  <td><?php echo $row->nama;?></td>
							  <td><?php echo $row->cara_bayar;?></td>
							  <td><?php echo $row->id_poli.'-'.$row->nm_poli;?></td>
							  <td>
							  <?php if($row->status == NULL){ ?>
							     <button class="btn btn-danger btn-xs" onclick="cek('<?php echo $row->no_kwitansi ?>')">Batal Kwitansi</button>
							     <?php
							     }
							     ?>		
								<!-- <a href="<?php //echo site_url('irj/rjckwitansi/cetak_kwitansi_kt/'.$no_register.'/1'); ?>" target="_blank" class="btn btn-primary btn-xs" style="width:63px;">Cetak Ulang 1</a>	
								<a href="<?php //echo site_url('irj/rjckwitansi/cetak_faktur_kt/'.$no_register); ?>" target="_blank" class="btn btn-primary btn-xs" style="width:63px;">Cetak Ulang 2</a>	 -->
								<!-- <a href="<?php echo site_url('irj/rjckwitansi/cetak_sebelum_poli/'.$no_register); ?>" target="_blank" class="btn btn-primary btn-xs" style="width:63px;">Cetak Sebelum Poli</a>	
								<a href="<?php echo site_url('irj/rjckwitansi/cetak_setelah_poli/'.$no_register); ?>" target="_blank" class="btn btn-primary btn-xs" style="width:63px;">Cetak Setelah Poli</a> -->
								<?php if($row->referensi == 1){ ?>
									<a href="<?php echo site_url('irj/rjckwitansi/cetak_sebelum_poli/'.$no_register); ?>" target="_blank" class="btn btn-primary btn-xs">Cetak Sebelum Poli</a>	
							     <!-- <button class="btn btn-primary btn-xs" onclick="cek_double('<?php echo $row->no_kwitansi ?>')">Batal Kwitansi Double</button> -->
							     <?php } else if($row->referensi == 2){ ?>
									<a href="<?php echo site_url('irj/rjckwitansi/cetak_setelah_poli/'.$no_register); ?>" target="_blank" class="btn btn-primary btn-xs">Cetak Setelah Poli</a>
								 <?php } else { ?>
								<?php } ?>					
							  </td>
							</tr>
						<?php
							}
						?>
						</tbody>
					</table>
					
					<?php
					//echo $this->session->flashdata('message_nodata');
					?>
				</div>
			</div><!-- /panel body -->
        </div><!-- /.box-body -->	
</section>

<?php
    if ($role_id == 1) {
        $this->load->view("layout/footer_left");
    } else {
        $this->load->view("layout/footer_horizontal");
    }
?>
